...
(
    ...
    (
        ...
        where
            event_date_hk between '2021-01-01' and '2025-11-05'
    ...
    (
        select
            app_id,uuid,standard_order_date,order_status
             ,case when subscription_period ='lifetime' then '2025-11-10' else standard_order_expire_date end standard_order_expire_date
        from `dataintegration-265403.dwd.dwd_dap_subscription_trial_subscription_retention_daily`
        where
            app_id = 'AirBrush' and event_date_hk='2025-11-10'
...
select h.app_name,h.event_date,h.user_pseudo_id,h.country,h.has_trial,h.has_paid
    ,h.last_order_expire_date,DATE_DIFF(h.event_date,h.last_order_expire_date,day) expire_days
...
--             ,max(if(t.user_pseudo_id is not null,1,0)) today_sub
    ...
    (
        ...
        from `dataintegration-265403.stat.stat_active_advice_detail_d`
        where
            event_date_hk between '2021-01-01' and '2025-11-02'
...
--         from `dataintegration-265403.analytics.dwd_dzp_events_function`('2021-01-01','2025-11-02','airbrush',false)
--         where
...
--     )t on u.user_pseudo_id = t.user_pseudo_id and u.event_date=t.event_date
    ...
    (
        ...
        and event_date between '2021-01-01' and '2025-11-02'
        group by 1,2
...
--     ,d.today_sub
    ...
    ,h.expire_days,h.active_days expire_active_days
from dau_type d
...
where event_date between '2021-03-01' and '2025-10-31'
group by 1,2,3,4,5,6
