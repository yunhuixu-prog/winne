drop table if exists `dataintegration-265403.temp.winne_temp_cuxiao_new_year_source`;
create table if not exists `dataintegration-265403.temp.winne_temp_cuxiao_new_year_source` as
...
(
    ...
    from dataintegration-265403.temp.winne_temp_cuxiao_new_year_source
    where
        (
            (event_date between '2021-12-29' and '2022-01-04') -- 21，周三-周二
            or (event_date between '2022-12-29' and '2023-01-04') -- 22，周四-周三
    ...
    select
        ...
        ,count(distinct case when event_name ='sub_to_paid' then user_pseudo_id end) sub_success_to_paid_uv
        ,sum(case when event_name = 'sub_to_paid' then payment_price_usd else 0 end) sub_success_to_paid_gmv
    from dataintegration-265403.temp.winne_temp_cuxiao_new_year_source --,unnest(split(source_00,',')) s
    where
        (
            (event_date between '2021-12-29' and '2022-01-04') -- 21，周三-周二
            or (event_date between '2022-12-29' and '2023-01-04') -- 22，周四-周三
