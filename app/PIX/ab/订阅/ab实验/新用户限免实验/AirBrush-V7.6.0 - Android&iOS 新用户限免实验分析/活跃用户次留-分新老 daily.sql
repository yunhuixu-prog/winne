...
WITH enter AS (
    SELECT 
       ...
       -- event_timestamp
 FROM `dataintegration-265403.analytics.dwd_dzp_events_function`('2025-04-03' ,'2025-04-20','airbrush', false) -- 此处用false 速度会更快
...
,act as (
    ...
    where  
        event_date_hk between '2025-04-03' and'2025-04-20'
...
SELECT 
    ...
    COUNT(DISTINCT CASE WHEN DATE_DIFF(a.event_date_hk, o.event_date_hk, DAY) = 0 THEN o.device_id END) AS re0,
    COUNT(DISTINCT CASE WHEN DATE_DIFF(a.event_date_hk, o.event_date_hk, DAY) = 1 THEN o.device_id END) AS re1
...
FROM rs o -- 需要求留存的用户
LEFT JOIN act a ON o.device_id = a.device_id AND a.event_date_hk >= o.event_date_hk
where o.event_date_hk between '2025-04-03' and'2025-04-19'
GROUP BY 1,2,3,4
ORDER BY 1,2