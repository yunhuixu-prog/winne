...
where
    ...
    and _table_suffix between'20260107' and '20260122'
   and date(timestamp_micros(event_timestamp),'Asia/Singapore')  between  '2026-01-08' and'2026-01-21'
...
,act as (
    ...
    where
        event_date_hk between '2026-01-08' and '2026-01-21'
        and  app_name = 'AirBrush'
)
,enter AS (
-- 取活跃用户中有进入实验的用户
    select *
    ,case when ab_code in ('28577','28574') then '对照组'
         when ab_code in ('28578','28575') then '实验组A'
         when ab_code in ('28579','28576') then '实验组B'
        end code
    from
    (
        select
            e.device_id,e.platform,e.ab_code,e.enter_abtest_date,fa.is_new,e.event_timestamp,fa.country
...
from (select distinct device_id,code,platform from enter) a  -- where platform='ANDROID'
left join fe_sub b
on a.device_id=b.device_id
-- where a.is_new=1
group by 1,2
