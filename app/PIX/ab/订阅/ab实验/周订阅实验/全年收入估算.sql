...
-- 修改实验abcode：批量替换 '28577','28578','28579','28574','28575','28576'
DECLARE mDATE_ABTEST_START DATE DEFAULT '2026-01-08';
DECLARE mDATE_ABTEST_END DATE DEFAULT '2026-02-02';
...
where
    ...
    and func.getParams(event_params,'current_abcode').string_value in  ('28577','28578','28579','28574','28575','28576')
...
,act as (
    -- 活跃表
    select
        event_date_hk, user_pseudo_id, platform, uuid, is_new, country
...
(
    select
        e.device_id,e.user_pseudo_id,e.platform,e.abcode,e.enter_abtest_date,fa.uuid,fa.is_new enter_new,fa.country,e.event_timestamp
...
,fe as( -- 限制进入实验的人,且实验触发日期在进入实验之后
    select
        a.*except(abcode,country),b.abcode,b.enter_abtest_date,b.enter_new,b.country
    from eves a
...
,ads_ab as (
    ...
    select
        ad.*
        ,e.abcode,e.enter_abtest_date,e.enter_new,e.country
    from  ads ad
    join `dataintegration-265403.temp.dau_and_bookings_predict_365_enter_abtest_dau_1` e
...
--                     when order_status= 0 and next_standard_order_date is not null then device_id end) sub_success_to_paid_uv
    ...
    (
        select
            a.enter_abtest_date,a.abcode,a.device_id,a.country
...
--         where platform='IOS' or (platform='ANDROID' and enter_new=0)
    )
    group by 1,2,3,4
