...
,enter AS (
-- 取活跃用户中有进入实验的用户
    select *,case when ab_code in ('11159','11162') then '对照组'
...
from (select distinct device_id,code from enter where platform='ANDROID') a
left join fe_sub b
on a.device_id=b.device_id
group by 1,2
...
select case when a.abcode in ('11159','11162') then '对照组'
             when a.abcode in ('11160','11163') then '实验组A'
             when a.abcode in ('11161','11164') then '实验组B'
            end code
     ,count(distinct a.user_pseudo_id) computer_uv,sum(b.value) total
     ,round(AVG(coalesce(b.value,0)),6) mean,round(STDDEV(coalesce(b.value,0)),4) std
from (select distinct user_pseudo_id,abcode from enter where platform='ANDROID') a
left join ads_ab b
on a.user_pseudo_id=b.user_pseudo_id --and a.abcode=b.abcode
group by 1