drop table if exists `dataintegration-265403.temp.livex_behavior_analysis_winne`;
create table if not exists `dataintegration-265403.temp.livex_behavior_analysis_winne` as

with eves as (
select
    event_date date,country,platform,user_pseudo_id,is_ua,user_type,is_new,app_version
    ,event_name,event_timestamp,action,function_level,function0,function1,function2,function3,function4,paid_type
FROM
  `dataintegration-265403.dwd.dwd_dzp_behavior_ab_edit_detail`
WHERE
    event_date between '2026-02-17' and '2026-03-15'
...
from `dataintegration-265403.analytics.dwd_dzp_events_function`('2026-02-17','2026-03-15','airbrush',false)
where
...
--    and date(timestamp_micros(event_timestamp),'Asia/Singapore')  between  '2026-02-17' and'2025-12-01'
)
...
select function_level
       ,case when ab_code in ('28656') then '对照组'
         when ab_code in ('28657') then '实验组A'
         when ab_code in ('28658') then '实验组B'
        end code
       ,platform,paid_type
...
from `dataintegration-265403.temp.livex_behavior_analysis_winne`
where function_level in ('1','2','3')
