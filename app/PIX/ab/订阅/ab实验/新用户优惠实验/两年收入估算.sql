...
-- 修改实验abcode：批量替换 '28656','28657','28658'
DECLARE mDATE_ABTEST_START DATE DEFAULT '2026-02-17';
DECLARE mDATE_ABTEST_END DATE DEFAULT '2026-03-17';
...
where
    ...
    and func.getParams(event_params,'current_abcode').string_value in  ('28656','28657','28658')
...
(
    select
        e.device_id,e.user_pseudo_id,e.platform,e.abcode,e.enter_abtest_date,fa.uuid,fa.is_new enter_new,e.event_timestamp
...
select
    ...
    ,platform,user_pseudo_id,geo.country country,event_timestamp
    ,func.getUserprop(user_properties,'device_id').string_value as device_id
...
,fe as( -- 限制进入实验的人,且实验触发日期在进入实验之后
    select
        a.*except(abcode),b.abcode,b.enter_abtest_date,b.enter_new
    from eves a
...
,ads_ab as (
    ...
    from  ads ad
    join
       (select user_pseudo_id,abcode,enter_new,min(enter_abtest_date) enter_abtest_date from `dataintegration-265403.temp.dau_and_bookings_predict_365_enter_abtest_dau_1` group by 1,2,3) e
...
--                     when order_status= 0 and next_standard_order_date is not null then device_id end) sub_success_to_paid_uv
    ...
    (
        select
            a.platform,a.enter_abtest_date,a.abcode,a.device_id
            ,b.standard_order_date,b.original_order_id
            ,case when b.sku in ('airbrush.subs.month12.func00.lev0.campaign.newuserdiscount.ver0','com.meitu.airbrush.autorenew.vip7','com.meitu.airbrush.autorenew.vip8'
                                ,'airbrush.sub.mon12.fun00.lev00.newdis30','com.meitu.airbrush.subscription.vip16','com.meitu.airbrush.subscription.vip17')
                                then 'discount sku'
                else b.subscription_period
