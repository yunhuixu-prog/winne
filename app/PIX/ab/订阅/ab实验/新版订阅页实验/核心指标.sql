...
where
    ...
    and _table_suffix between'20260326' and '20260414'
   and date(timestamp_micros(event_timestamp),'Asia/Singapore')  between  '2026-03-27' and'2026-04-13'
...
where
    ...
    and func.getParams(event_params,'current_abcode').string_value in  ('28755','28756','28757','28758','28759','28760')
      and _table_suffix between '20260326' and '20260414'
   and date(timestamp_micros(event_timestamp),'Asia/Singapore')  between  '2026-03-27' and'2026-04-13'
)
,act as (
    ...
    where
        event_date_hk between '2026-03-27' and'2026-04-13'
...
--     where b.enter_abtest_date  <= a.date -- 事件发生的日期均 >= 进入实验日期
    where b.event_timestamp-15000000 <= a.event_timestamp
...
,paid as (
    ...
    and standard_order_date >= '2026-03-27'
    and order_status in (0,1,2)
)
select
    a.date,a.platform
    ,case when ab_code in ('28755','28758') then '对照组'
         when ab_code in ('28756','28759') then '实验组A'
         when ab_code in ('28757','28760') then '实验组B'
        end code
    ,a.ab_code
    ,a.is_new
...
--     ,a.enter_abtest_date
    ...
    ,count(distinct case when (a.event_name = 'w_subscription_success' and c.order_status in (1,2)) then c.device_id
                when  ( a.event_name = 'w_subscription_success' and c.order_status= 0 and c.next_standard_order_date is not null ) then  c.device_id end )  paid_uv
