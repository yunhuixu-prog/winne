...
where
    ...
    and _table_suffix between'20250908' and '20250923'
   and date(timestamp_micros(event_timestamp),'Asia/Singapore')  between  '2025-09-09' and'2025-09-21'
...
where
    ...
    and func.getParams(event_params,'current_abcode').string_value in  ('11402','11403','11404','11405')
      and _table_suffix between '20250908' and '20250923'
   and date(timestamp_micros(event_timestamp),'Asia/Singapore')  between  '2025-09-09' and'2025-09-21'
)
,act as (
    ...
    where
        event_date_hk between '2025-09-09' and '2025-09-21'
...
,paid as (
    ...
    and standard_order_date >= '2025-09-09'
...
--     a.date,
    a.platform
    ,case when ab_code in ('11402','11404') then '对照组'
         when ab_code in ('11403','11405') then '实验组'
        end code
    ,a.ab_code
--     ,a.enter_abtest_date
    -- 付费
    ...
    ,case when a.source_module in ('p_edit') then 'p_edit' else 'else' end source_0
    ,case when a.source_module in ('p_edit') and s0 in ('f_wrinkle','f_concealer','f_brighten','f_matte','f_skin_tone','f_details','f_contour','f_texture') then 'f_skin'
          when a.source_module in ('p_edit') then s0
    else 'else' end source_1
...
from fe a,unnest(split(coalesce(source_0,'others'),',')) s0
    left join
    (   select
