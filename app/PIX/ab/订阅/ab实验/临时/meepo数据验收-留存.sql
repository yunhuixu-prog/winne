WITH enter AS (
    SELECT
       ...
       -- event_timestamp
 FROM `dataintegration-265403.analytics.dwd_dzp_events_function`('2025-06-22' ,'2025-08-06','airbrush', false) -- 此处用false 速度会更快
...
,act as (
    ...
    where
        event_date_hk between '2025-06-22' and '2025-08-07'
        and  app_name = 'AirBrush'
)
,ad AS (
-- 取活跃用户中有进入实验的用户
    select
        e.abcode,e.enter_abtest_date,e.user_pseudo_id,fa.is_new enter_new
    from  act fa
    join
...
--     o.event_date_hk,
    COUNT(DISTINCT o.user_pseudo_id) AS enter_abtest_uv,
...
where o.event_date_hk between '2025-06-22' and'2025-08-06'
