-- 用的时候看一下要不要用价格实验更新后的以免进入abtest报重复了
-- 实验周期选择：最好选择较为稳定的一段时间，不要从最开始取
-- 修改实验abcode：批量替换 '11072','11073','11074','11075','11076','11077'
-- 部分参数修改：84-86有一些样本量级的参数限制，可以修改
DECLARE mDATE_ABTEST_START DATE DEFAULT '2025-04-07';
DECLARE mDATE_ABTEST_END DATE DEFAULT '2025-07-25';
...
,enter as (
    -- 进入实验且当天活跃
    select
        fa.user_pseudo_id,fa.platform,fa.device_id
...
(
    select
        enter_abtest_date,
        abcode,
        COUNT(DISTINCT device_id) AS control_active0
    from enter
    group by 1,2
) a
join
(
    SELECT
        ...
        COUNT(DISTINCT device_id) AS control_activex
    FROM rs
    GROUP BY 1,2,3,4,5
) b
on a.enter_abtest_date=b.enter_abtest_date and a.abcode=b.abcode
...
-- 进入实验且当天活跃
select
    fa.user_pseudo_id,fa.platform,fa.uuid
    ,e.abcode,e.enter_abtest_date,e.device_id
from  act fa
join
   enter_pre e ON e.user_pseudo_id = fa.user_pseudo_id and e.enter_abtest_date = fa.event_date_hk
where e.user_pseudo_id is not null
...
select
    b.abcode,sum(abtest_dau) abtest_dau,sum(dau) dau
