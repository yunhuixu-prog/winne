...
drop table if exists dataintegration-265403.temp.winne_temp_brazil_free_analysis;
create table dataintegration-265403.temp.winne_temp_brazil_free_analysis as
...
with active AS (
    select app_name,
        ...
        where
            event_date_hk between '2026-01-01' and date_add('2026-03-31',interval 7 day)
            and app_name = 'AirBrush'
        group by 1,2,3,4
)
,function_use as (
    select
        event_date,user_pseudo_id,function1,function2
        ,count(case when action='点击' then 1 end) enter_pv
    ...
    WHERE
        ...
        and (
            (function_level = '2' and function1 in ('Retouch','Edit')) 
            or (function_level = '1' and function1 not in ('Retouch','Edit'))
            )
...
,sub AS (
    select event_date,user_pseudo_id
       ...
       ,MAX(case when event_name='sub_to_paid' then 1 end) is_sub_to_paid
    ...
    where source_module != 'all'
        and event_date between '2026-01-01' and '2026-03-31'
...
where a.event_date_hk between '2026-01-01' and '2026-03-31'
...
from dataintegration-265403.temp.winne_temp_brazil_free_analysis
