drop table if exists `dataintegration-265403.temp.ai_beta_behavior_analysis_winne`;
create table if not exists `dataintegration-265403.temp.ai_beta_behavior_analysis_winne` as
...
WHERE
    event_date between '2025-10-31' and '2025-11-24'
...
from `dataintegration-265403.analytics.dwd_dzp_events_function`('2025-10-31','2025-11-24','airbrush',false)
where
    event_name = 'abcode_enter_test'
    and func.getParams(event_params,'current_abcode').string_value in  ('28422','28423','28424')
--       and _table_suffix between '20250719' and '20250727'
--    and date(timestamp_micros(event_timestamp),'Asia/Singapore')  between  '2025-10-31' and'2025-11-24'
)
...
select function_level
       ,case when ab_code in ('28422') then '0:对照组'
         when ab_code in ('28423') then '1:中立组'
         when ab_code in ('28424') then '2:实验组'
        end code
--        ,platform
...
from `dataintegration-265403.temp.ai_beta_behavior_analysis_winne`
