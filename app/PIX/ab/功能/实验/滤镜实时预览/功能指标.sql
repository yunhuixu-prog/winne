drop table if exists `dataintegration-265403.temp.filter_behavior_analysis_winne`;
create table if not exists `dataintegration-265403.temp.filter_behavior_analysis_winne` as
...
WHERE
    event_date between '2025-11-13' and '2026-01-11'
...
select
    distinct
   date(timestamp_micros(event_timestamp),'Asia/Singapore')  date, user_pseudo_id, event_timestamp
    ,geo.country country
    ,platform
...
from `dataintegration-265403.analytics.dwd_dzp_events_function`('2025-11-13','2026-01-11','airbrush',false)
where
    event_name = 'abcode_enter_test'
    and func.getParams(event_params,'current_abcode').string_value in  ('28465','28466','28467')
    and `dataintegration-265403.func`.compare_is_greater_or_equal_version(app_info.version,'7.22.0')
--       and _table_suffix between '20250719' and '20250727'
--    and date(timestamp_micros(event_timestamp),'Asia/Singapore')  between  '2025-11-13' and'2026-01-11'
)
,fe as( -- 限制进入实验的人,且实验触发日期在进入实验之后
    ...
    where b.event_timestamp  <= a.event_timestamp -- 事件发生的日期均 >= 进入实验日期
)
...
select function_level
       ,case when ab_code in ('28465') then '0:对照组'
         when ab_code in ('28466') then '1:中立组'
         when ab_code in ('28467') then '2:实验组'
        end code
       ,platform
...
from `dataintegration-265403.temp.filter_behavior_analysis_winne`
where function_level in ('1') and platform='ANDROID'
--     and function1='Filters'
