...
where
    ...
    and _table_suffix between'20251112' and '20260112'
...
,act as (
    ...
    where
        event_date_hk between '2025-11-13' and '2026-01-11'
...
-- 取活跃用户中有进入实验的用户
    ...
    (
        select
            e.device_id,e.platform,e.ab_code,e.enter_abtest_date,fa.is_new,e.event_timestamp
...
from (select distinct device_id,code,is_new from enter where platform='ANDROID') a
left join fe_sub b
on a.device_id=b.device_id
group by 1
order by 1