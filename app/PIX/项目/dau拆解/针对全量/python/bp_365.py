def common_function(ds, **kwargs):
    ...
    def model_train(d, e_d, install_days_type, sub_type):
        ...
        query = """
                        ...
                        where date between '2023-02-01' and '2023-03-30'
                            and sub_365=0 
                            and rand()<0.015*least(500000/{1},1)
                            and sub_type={3} 
                            -- and is_new=0
                            and install_days_type = {2}
        ...
        del X_no_sub_now, xtest, ytest, y_no_sub_now

        return stacking

    def read_data(d, e_d, days, last_active_days_type, install_days_type, sub_type):
        print('真实样本')
        client = bigquery.Client()
        # 选择投放未满90天的数据
        query_real = """
    ...
    def real_predict(X_no_sub_now_real, stacking):
        # 真实数据预测(拿历史数据预估，仅测试)
        y_no_sub_now_predit_real = stacking.predict_proba(X_no_sub_now_real)[:, 1]
        return y_no_sub_now_predit_real

    def metrcis_cal(data_pre_real, y_no_sub_now_real, y_no_sub_now_predit_real, e_d, days, last_active_days_type, install_days_type, sub_type):
    ...
    models = []
    for i in range(4):
        stacking = model_train(d, e_d, install_days_type, sub_type)
    ...
    #         real_predict(stacking, d, e_d, days, last_active_days_type, install_days_type, sub_type)
    #         gc.collect()
    # read data

    for days in range(0, 1):
        for last_active_days_type in range(1, 10):
            X_no_sub_now_real, y_no_sub_now_real, data_pre_real = read_data(d, e_d, days, last_active_days_type, install_days_type,  sub_type)
            if X_no_sub_now_real.shape[0] == 0:
                continue
            outs = []
            for i in range(len(models)):
                ...
                outs.append(out)
            fina_pred = bagging_final_res(outs,'max')
            del outs, out, X_no_sub_now_real
            print(fina_pred.shape, y_no_sub_now_real.shape)
            metrcis_cal(data_pre_real, y_no_sub_now_real, fina_pred, e_d, days, last_active_days_type, install_days_type,  sub_type)
            del X_no_sub_now_real, y_no_sub_now_real, data_pre_real, fina_pred
            gc.collect()
