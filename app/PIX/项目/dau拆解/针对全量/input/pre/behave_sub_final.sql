...
-- delete from beautyplus-bc0ed.temp.dws_dz_his_split_final_user_behave where date between mDATE_START and mDATE_END;
...
select g.event_date_hk date,g.app_name,g.user_pseudo_id,gi.gid,CAST(FLOOR(RAND() * 6) AS INT64) bucket
        ...
        ,fac.active_365,fac.active_90,fac.active_30,fac.active_7
from
(
...
--          ,coalesce(max(c.credit_revenue),0) current_credit_revenue
         ...
         ,coalesce(max(sub_30),0) sub_30
         ,coalesce(max(sub_7),0) sub_7
         ,coalesce(sum(sub_revenue_365),0) sub_revenue_365
         ,coalesce(sum(sub_revenue_90),0) sub_revenue_90
         ,coalesce(sum(sub_revenue_30),0) sub_revenue_30
         ,coalesce(sum(sub_revenue_7),0) sub_revenue_7
