...
drop table if exists airbrush-1324.temp.ads_dz_his_split_predict_sub_365_proba;
create table airbrush-1324.temp.ads_dz_his_split_predict_sub_365_proba as
...
from airbrush-1324.temp.ads_dz_his_split_predict_sub_365_proba
group by 1
order by 1
;
...
(
    select date,uuid,sub_365,sub_type,predit_sub_365_proba
         ,install_days_type
    from airbrush-1324d.temp.ads_dz_his_split_predict_sub_365_proba
    where date between '2023-05-01' and '2023-05-01'
        and sub_type='else'
) a
left join
(
    select uuid
    from `dataintegration-265403.stat.stat_active_advice_detail_d`
    where event_date_hk between '2023-05-02' and '2023-05-02' and app_name in ('AirBrush')
    group by uuid
) b
on a.uuid=b.uuid
where b.uuid is not null
...
-- drop table if exists airbrush-1324.temp.dws_dz_his_split_predict_thred;
-- create table airbrush-1324.temp.dws_dz_his_split_predict_thred as

DECLARE thred FLOAT64 DEFAULT 0.0;

WHILE thred <= 1.0 DO
delete from airbrush-1324.temp.dws_dz_his_split_predict_thred where thredshod = thred;
insert into airbrush-1324.temp.dws_dz_his_split_predict_thred
...
select 'AirBrush' app_name,date,sub_type,install_days_type,is_active_af_1,round(thredshod,2) thredshod
    ,uv,real_sub_365_uv,real_and_predict_sub_365_uv,predict_sub_365_uv
from airbrush-1324.temp.dws_dz_his_split_predict_thred
order by 1,2,3,4,5
