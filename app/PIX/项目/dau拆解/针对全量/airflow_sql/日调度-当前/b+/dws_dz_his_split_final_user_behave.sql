...
-- drop table if exists beautyplus-bc0ed.temp.dws_dz_his_split_final_user_behave;
-- create table beautyplus-bc0ed.temp.dws_dz_his_split_final_user_behave as

DECLARE mDATE_START DATE DEFAULT '{{ (execution_date + macros.timedelta(hours=8) - macros.timedelta(days=2)).strftime("%Y-%m-%d") }}';
...
delete from beautyplus-bc0ed.temp.dws_dz_his_split_final_user_behave where date = mDATE;
insert into beautyplus-bc0ed.temp.dws_dz_his_split_final_user_behave
...
(
    ...
    where event_date_hk>='2024-01-01' and event_date_hk = mDATE and app_id in ('BeautyPlus')
...
--         , sum(force_grow_sub_page_enter) force_grow_sub_page_enter, sum(force_grow_sub_page_click) force_grow_sub_page_click, sum(subscript_grow_sub_page_enter) subscript_grow_sub_page_enter, sum(subscript_grow_sub_page_click) subscript_grow_sub_page_click, sum(other_grow_sub_page_enter) other_grow_sub_page_enter, sum(other_grow_sub_page_click) other_grow_sub_page_click, sum(grow_max_impression_pv) grow_max_impression_pv, sum(grow_impression_pv) grow_impression_pv, sum(grow_click_pv) grow_click_pv
        ...
        , sum(pay_function_click_pv_30) pay_function_click_pv_30, sum(free_function_click_pv_30) free_function_click_pv_30, sum(free_function_save_pv_30) free_function_save_pv_30
        , sum(pay_function_click_pv_60) pay_function_click_pv_60, sum(free_function_click_pv_60) free_function_click_pv_60, sum(free_function_save_pv_60) free_function_save_pv_60
        , sum(pay_function_click_pv_90) pay_function_click_pv_90, sum(free_function_click_pv_90) free_function_click_pv_90, sum(free_function_save_pv_90) free_function_save_pv_90
--         , sum(grow_pay_function_click_pv) grow_pay_function_click_pv, sum(grow_free_function_click_pv) grow_free_function_click_pv, sum(grow_free_function_save_pv) grow_free_function_save_pv
        ...
        , sum(pay_duffle_click_pv_30) pay_duffle_click_pv_30, sum(free_duffle_click_pv_30) free_duffle_click_pv_30, sum(free_duffle_save_pv_30) free_duffle_save_pv_30
        , sum(pay_duffle_click_pv_60) pay_duffle_click_pv_60, sum(free_duffle_click_pv_60) free_duffle_click_pv_60, sum(free_duffle_save_pv_60) free_duffle_save_pv_60
        , sum(pay_duffle_click_pv_90) pay_duffle_click_pv_90, sum(free_duffle_click_pv_90) free_duffle_click_pv_90, sum(free_duffle_save_pv_90) free_duffle_save_pv_90
--         , sum(grow_pay_duffle_click_pv) grow_pay_duffle_click_pv, sum(grow_free_duffle_click_pv) grow_free_duffle_click_pv, sum(grow_free_duffle_save_pv) grow_free_duffle_save_pv
        , sum(function_num) function_num, sum(function_num_pre) function_num_pre, sum(function_num_30) function_num_30, sum(function_num_60) function_num_60, sum(function_num_90) function_num_90, sum(grow_function_num) grow_function_num
--         , sum(grow_edit_enter_pv) grow_edit_enter_pv, sum(grow_edit_save_pv) grow_edit_save_pv, sum(grow_take_photo_pv) grow_take_photo_pv, sum(grow_take_photo_save_pv) grow_take_photo_save_pv, sum(grow_selftake_enter_pv) grow_selftake_enter_pv, sum(grow_take_video_pv) grow_take_video_pv, sum(grow_take_video_save_pv) grow_take_video_save_pv
