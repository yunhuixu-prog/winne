...
def metric_cal(data_pre_real, y_no_sub_now_real, y_no_sub_now_predit_real, e_d, sub_type
    ...
    # airbrush-1324.temp.ads_dz_his_split_predict_sub_365
...
def table_initial(e_d, sub_type, install_days_type_1, install_days_type_2
    ...
    # 删除输出表观测日期数据
    query_delete = """
                        delete from airbrush-1324.temp.ads_dz_his_split_predict_sub_365
...
def load_data(e_d, sub_type, install_days_type_1, install_days_type_2
    ...
    query_drop = """ drop table if exists airbrush-1324.temp.dws_dz_his_split_final_user_behave_extract_batch """
    drop_job = client.query(query_drop)
    drop_job.result()

    # 选择要预测的数据
    query_create = """
                create table airbrush-1324.temp.dws_dz_his_split_final_user_behave_extract_batch as 
    ...
    query_test = """
                        select *
                        from airbrush-1324.temp.dws_dz_his_split_final_user_behave_extract_batch
                        limit 10
                                     """
    ...
    os.system("bash /home/yunhui/ab/load_predict_data_ab.sh")
...
def predict_data(e_d, sub_type, install_days_type_1, install_days_type_2
    ...
    # 读取所有json文件
    files = os.listdir('/home/yunhui/ab/data/predict/predict_data_ab')
    raw_data = pd.DataFrame()
    for file in files:
        print(f'read file:{file}')
        reader = pd.read_json('/home/yunhui/ab/data/predict/predict_data_ab/' + file, chunksize=5000, lines=True)
        batch_num = 0
        for batch_data in reader:
            # print(batch_num)
            batch_num = batch_num + 1
