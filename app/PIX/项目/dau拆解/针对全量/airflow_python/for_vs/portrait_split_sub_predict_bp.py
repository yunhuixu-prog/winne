...
def model_train(d, e_d, sub_type, install_days_type_1, install_days_type_2
    ...
    query_cnt_1 = """
                ...
                (
                    ...
                    where date between DATE_SUB({0}, INTERVAL 366+180 DAY) and DATE_SUB({0}, INTERVAL 366 DAY)
                        and date>='2023-01-01'
    ...
    query_drop = """ drop table if exists beautyplus-bc0ed.temp.dws_dz_his_split_final_user_behave_model """
    drop_job = client.query(query_drop)
    drop_job.result()

    query_create = """
                    create table beautyplus-bc0ed.temp.dws_dz_his_split_final_user_behave_model as 
    ...
    os.system("bash /home/yunhui/bp/load_train_data_bp.sh")

    files = [pos_json for pos_json in os.listdir('/home/yunhui/bp/data/train/train_data_bp/') if
    ...
    # 循环读取和合并每个JSON文件
    for file in files:
        file_path = os.path.join('/home/yunhui/bp/data/train/train_data_bp/', file)
    ...
    raw_data = (raw_data.drop(['sub_7', 'sub_30', 'sub_90', 'date'], axis=1))
    ...
    dump(stacking, '/home/yunhui/bp/model/stacking_' + model_name + '.joblib')
...
def real_predict(X_no_sub_now_real, bucket, model_name):
    # 真实数据预测(拿历史数据预估，仅测试)
    model_name = model_name + '_bucket_' + str(bucket)
    stacking = load('/home/yunhui/bp/model/stacking_' + model_name + '.joblib')
...
def metric_cal(data_pre_real, y_no_sub_now_real, y_no_sub_now_predit_real, e_d, sub_type
    ...
    # beautyplus-bc0ed.temp.ads_dz_his_split_predict_sub_365
    table_id_user = 'temp.ads_dz_his_split_predict_sub_365'
...
def table_initial(e_d, sub_type, install_days_type_1, install_days_type_2
    ...
    # 删除输出表观测日期数据
    query_delete = """
                        delete from beautyplus-bc0ed.temp.ads_dz_his_split_predict_sub_365
...
def load_data(e_d, sub_type, install_days_type_1, install_days_type_2
    ...
    query_drop = """ drop table if exists beautyplus-bc0ed.temp.dws_dz_his_split_final_user_behave_extract_batch """
    drop_job = client.query(query_drop)
    drop_job.result()

    # 选择要预测的数据
    query_create = """
                create table beautyplus-bc0ed.temp.dws_dz_his_split_final_user_behave_extract_batch as 
    ...
    query_test = """
                                select *
                                from beautyplus-bc0ed.temp.dws_dz_his_split_final_user_behave_extract_batch
                                limit 10
                                             """
    ...
    if data_test.shape[0] == 0:
        ...
        if 'homepage_banner_click_pv' not in data_test.columns:
            print(f'异常！！表格指标错误')
        else:
            print(f'表格正常')

    os.system("bash /home/yunhui/bp/load_predict_data_bp.sh")
...
def predict_data(e_d, sub_type, install_days_type_1, install_days_type_2
    ...
    # 读取所有json文件
    files = os.listdir('/home/yunhui/bp/data/predict/predict_data_bp')
    raw_data = pd.DataFrame()
    for file in files:
        print(f'read file:{file}')
        reader = pd.read_json('/home/yunhui/bp/data/predict/predict_data_bp/' + file, chunksize=5000, lines=True)
        batch_num = 0
        for batch_data in reader:
            # print(batch_num)
            batch_num = batch_num + 1
...
if __name__ == '__main__':
    ...
    flag = 0
    for i in range(1, train_bucket):
        if os.path.exists('/home/yunhui/bp/model/stacking_' + model_name + '_bucket_' + str(i) + '.joblib'):
            print(f'has model:{i}')
        else:
