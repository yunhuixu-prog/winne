...
drop table if exists airbrush-1324.temp.dws_dz_dau_split_final_user_behave;
create table airbrush-1324.temp.dws_dz_dau_split_final_user_behave as

-- delete from airbrush-1324.temp.dws_dz_dau_split_final_user_behave where date between mDATE_START and mDATE_END;
-- insert into airbrush-1324.temp.dws_dz_dau_split_final_user_behave
...
(
    ...
    where event_date_hk between mDATE_START and mDATE_END
        and app_name in ('AirBrush')  -- 'AirBrush',
    group by 1,2,3,4
)
,
his_sub_event as
(
    ...
    where event_date_hk between mDATE_START and mDATE_END and app_id in ('AirBrush')
)
,
sub_event as
(
...
(
    select
        app_id app_name,uuid,standard_order_date,order_status,max(payment_price_usd) sub_revenue
        ,max(case when subscription_period ='lifetime' then mDATE_END else standard_order_expire_date end) standard_order_expire_date
    from `dataintegration-265403.subscription.dwd_trial_subscription_retention_daily_temp`
    where
        app_id in('AirBrush')
...
--     from beautyplus-bc0ed.temp.dws_dz_dau_split_user_behave
...
select g.event_date_hk date,g.app_name,g.user_pseudo_id
        ...
        ,d.*except(event_date_hk,app_name,user_pseudo_id)
...
(
    select g.event_date_hk,g.app_name,g.user_pseudo_id
         ...
         ,coalesce(max(if(s.uuid is not null,1,0)),0) is_current_sub
         ,coalesce(max(s.sub_revenue),0) current_sub_revenue
         ,coalesce(max(if(st.uuid is not null,1,0)),0) is_current_trial_pre
         -- 预测指标
    ...
    left join (select * from sub_period where order_status in (1,2)) s
    on g.app_name=s.app_name and g.uuid=s.uuid and g.event_date_hk >= s.standard_order_date  and g.event_date_hk <= s.standard_order_expire_date
    left join (select * from sub_period where order_status in (0)) st
    on g.app_name=st.app_name and g.uuid=st.uuid and g.event_date_hk >= st.standard_order_date  and g.event_date_hk <= st.standard_order_expire_date
    group by 1,2,3
) g
