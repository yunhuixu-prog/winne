DECLARE mDATE_START DATE DEFAULT PARSE_DATE('%Y-%m-%d', '2023-04-01');
...
delete from beautyplus-bc0ed.temp.dws_dz_roi_predict_user_behave_month where date=mDATE;
insert into beautyplus-bc0ed.temp.dws_dz_roi_predict_user_behave_month

-- DECLARE mDATE DATE DEFAULT '2023-01-01';
-- drop table if exists beautyplus-bc0ed.temp.dws_dz_roi_predict_user_behave_month;
-- create table beautyplus-bc0ed.temp.dws_dz_roi_predict_user_behave_month as
...
(
    ...
    where event_date between date_sub(mDATE,interval 30 day) and mDATE and a.mark in (0, 1, 2)
)
-- ,
-- -- 近31天用户核心行为
-- core_behave as
...
--         FROM
--         behave_pre
--         WHERE mark_c in ('0','1') and action in ('点击', '进入', '保存', '拍摄')
...
(
    ...
    (
        select user_pseudo_id
                , sum (case when event_date between date_sub(mDATE, interval 30 day) and mDATE
...
select cast(mDATE as date) as date,g.Attributed_Touch_Date,g.user_pseudo_id
    ,a.active_days_90,a.active_days_60,a.active_days_31,a.active_days_14
