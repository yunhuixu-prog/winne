def common_function(ds, **kwargs):
    ...
    def model_train(d,e_d):
        ...
        query = """
                        ...
                        where types='new' and Attributed_Touch_Date between DATE_SUB({0}, INTERVAL 458 DAY) and DATE_SUB({0}, INTERVAL 365 DAY)
                            and sub_now=0 and sub_no_trial_365>0 and days between 7 and 365
                            and rand()<0.01*least(600000/{1},1)
                        ...
                        where types='new' and Attributed_Touch_Date between DATE_SUB({0}, INTERVAL 458 DAY) and DATE_SUB({0}, INTERVAL 365 DAY)
                            and sub_now=0 and sub_no_trial_365=0 and days between 7 and 365
                            and rand()<0.0001*least(600000/{1},1)
                             """.format(e_d, cnt)
        ...
        # beautyplus-bc0ed.temp.ads_dz_new_roi_predict_sample_evaluate
        table_id ='temp.ads_dz_new_roi_predict_sample_evaluate'
        credentials_path = os.environ['GOOGLE_APPLICATION_CREDENTIALS']
        credentials = service_account.Credentials.from_service_account_file(credentials_path)
        # 删除观测日期近7天数据
        query_delete = """
                delete from beautyplus-bc0ed.temp.ads_dz_new_roi_predict_sample_evaluate where date = {0}
                """.format(e_d)
    ...
    def real_predict(stacking,xgbr,d,e_d,diff1,diff2):
        ...
        # beautyplus-bc0ed.temp.ads_dz_new_roi_predict_user_predict_sub_revenue
        table_id_user ='temp.ads_dz_new_roi_predict_user_predict_sub_revenue'
        credentials_path = os.environ['GOOGLE_APPLICATION_CREDENTIALS']
        credentials = service_account.Credentials.from_service_account_file(credentials_path)

        # 删除观测日期近7天数据
        query_delete = """
                delete from beautyplus-bc0ed.temp.ads_dz_new_roi_predict_user_predict_sub_revenue 
    ...
    e_d='\''+d+'\''
    stacking,xgbr = model_train(d, e_d)
