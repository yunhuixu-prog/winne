def common_function(ds, **kwargs):
    ...
    def model_train(d, e_d, types):
        ...
        if types == 'classfication_second':
            ...
            query = """
                            ...
                            where types='new' and Attributed_Touch_Date between DATE_SUB({0}, INTERVAL 458 DAY) and DATE_SUB({0}, INTERVAL 365 DAY)
                                and sub_now=0 and sub_no_trial_365>0 and days between 7 and 365
                                and rand()<0.05*least(800000/{1},1)
        ...
        if types == 'others':
            ...
            query = """
                            ...
                            where types='new' and Attributed_Touch_Date between DATE_SUB({0}, INTERVAL 458 DAY) and DATE_SUB({0}, INTERVAL 365 DAY)
                                and sub_now=0 and sub_no_trial_365=0 and days between 0 and 6
                                and rand()<0.01*least(800000/{1},1) -- 可以调整
    ...
    def model_evaluate(d, e_d, stacking, stacking_, xgbr, thred, thred_diff):
        ...
        query = """
                ...
                where types='new' and Attributed_Touch_Date between DATE_SUB({0}, INTERVAL 458 DAY) and DATE_SUB({0}, INTERVAL 365 DAY)
                    and sub_now=0 and sub_no_trial_365>0 and days between 7 and 365
                    and rand()<0.05*least(700000/{1},1)
                ...
                where types='new' and Attributed_Touch_Date between DATE_SUB({0}, INTERVAL 458 DAY) and DATE_SUB({0}, INTERVAL 365 DAY)
                    and sub_now=0 and sub_no_trial_365=0 and days between 0 and 6
                    and rand()<0.008*least(700000/{1},1) -- 可以调整
                ...
                where types='new' and Attributed_Touch_Date between DATE_SUB({0}, INTERVAL 458 DAY) and DATE_SUB({0}, INTERVAL 365 DAY)
                    and sub_now=0 and sub_no_trial_365=0 and days between 7 and 365
                    and rand()<0.0002*least(700000/{1},1) -- 可以调整
                     """.format(e_d, cnt)
    ...
    def real_predict(stacking, stacking_, xgbr, d, e_d, diff1, diff2, thred_diff):
        ...
        if flag1_real.sum() > 0:
            ...
            del y_no_sub_now_second_predit_real
        else:
        ...
        del X_no_sub_now_real,y_no_sub_now_real
        ...
        #
        # # airbrush-1324.temp.ads_dz_roi_predict_new_sample_evaluate_test
        ...
        # airbrush-1324.temp.ads_dz_roi_predict_new_user_predict_sub_revenue_test
        ...
        # 删除观测日期近7天数据
        query_delete = """
                        delete from airbrush-1324.temp.ads_dz_roi_predict_new_user_predict_sub_revenue_test
    ...
    gc.collect()
    # evaluate = model_evaluate(d, e_d, stacking, stacking_, xgbr, thred, 0.1)
    # gc.collect()
    ...
    for diff1 in range(0, 3):
        for diff2 in range(0, 122):
            real_predict(stacking, stacking_, xgbr, d, e_d, diff1, diff2, 0.1)
            gc.collect()
