def common_function(ds, **kwargs):
    ...
    def model_train(d, e_d, types):
        ...
        if types == 'classfication_second':
            ...
            query = """
                            ...
                            where types='ua' and Attributed_Touch_Date between DATE_SUB({0}, INTERVAL 458 DAY) and DATE_SUB({0}, INTERVAL 365 DAY)
                                and sub_now=0 and sub_no_trial_365=0 and days between 7 and 365
                                and rand()<0.0008*least(800000/{1},1) -- 可以调整
                                 """.format(e_d, cnt)
        if types == 'others':
            query_cnt = """
                                select count(1)
            ...
            query = """
                            ...
                            where types='ua' and Attributed_Touch_Date between DATE_SUB({0}, INTERVAL 458 DAY) and DATE_SUB({0}, INTERVAL 365 DAY)
                                and sub_now=0 and sub_no_trial_365=0 and days between 0 and 6
                                and rand()<0.04*least(800000/{1},1) -- 可以调整
        ...
        for column in raw_data.columns:
            if column in ['sub_no_trial_365', 'sub_revenue_365', 'random']:
                continue
            elif column in ['brand', 'last_active_days', 'active_category', 'life_time_active_days', 'days']:
        ...
        if types == 'others':
            ...
            print(
                f"classification1 model input shape:{(X_no_sub_now.loc[X_no_sub_now['days'] <= 6, column_pre]).shape}")
    ...
    def model_evaluate(d, e_d, stacking, stacking_, xgbr, thred, thred_diff):
        ...
        query = """
                ...
                where types='ua' and Attributed_Touch_Date between DATE_SUB({0}, INTERVAL 458 DAY) and DATE_SUB({0}, INTERVAL 365 DAY)
                    and sub_now=0 and sub_no_trial_365=0 and days between 0 and 6
                    and rand()<0.04*least(700000/{1},1) -- 可以调整
                ...
                where types='ua' and Attributed_Touch_Date between DATE_SUB({0}, INTERVAL 458 DAY) and DATE_SUB({0}, INTERVAL 365 DAY)
                    and sub_now=0 and sub_no_trial_365=0 and days between 7 and 365
                    and rand()<0.0008*least(700000/{1},1) -- 可以调整
                     """.format(e_d, cnt)
        ...
        del data_pre,y_no_sub_now_predit,y_revenue_365_predict
    ...
    def real_predict(stacking, stacking_, xgbr, d, e_d, diff1, diff2, thred_diff):
        # 真实数据预测(拿历史数据预估，仅测试)
        print('真实样本')
        client = bigquery.Client()
        # 选择投放未满365天的数据
        ...
        column_af = []
        for column in X_no_sub_now_real.columns:
            if column in ['brand', 'last_active_days', 'active_category', 'life_time_active_days', 'days']:
                column_pre.append(column)
                column_af.append(column)
            elif '_7' in column or '_14' in column or '_31' in column or '_60' in column or '_90' in column:
                column_af.append(column)
        ...
        if flag1_real.sum() > 0:
            ...
            y_no_sub_now_second_predit_real = stacking_.predict_proba(X_no_sub_now_real.loc[flag1_real, column_af])[:,
                                              1]
            ...
            flag_real = (flag1_real * flag2_real) | (
                        (y_no_sub_now_predit_real >= thred_real) & (X_no_sub_now_real['days'] <= 6))
        else:
        ...
        del y_to_regression_predict_revenue_365_real
        ...
        #
        # # airbrush-1324.temp.ads_dz_roi_predict_sample_evaluate_test
        # table_id = 'temp.ads_dz_roi_predict_sample_evaluate_test'
        # credentials_path = os.environ['GOOGLE_APPLICATION_CREDENTIALS']
        # credentials = service_account.Credentials.from_service_account_file(credentials_path)
        # evaluate_all.to_gbq(table_id, project_id='airbrush-1324', if_exists='replace',  # append
        ...
        # 清除内存
        data_pre_real = pd.DataFrame()
        del data_pre_real,y_no_sub_now_predit_real,y_revenue_365_predict_real

        out_data_real.columns = ['date', 'Attributed_Touch_Date', 'days', 'user_pseudo_id', 'sub_revenue_365',
                                 'predit_no_trial_sub_365_proba', 'predit_no_trial_sub_365',
                                 'predict_sub_revenue_365']

        # airbrush-1324.temp.ads_dz_roi_predict_user_predict_sub_revenue_test
        table_id_user = 'temp.ads_dz_roi_predict_user_predict_sub_revenue_test'
        credentials_path = os.environ['GOOGLE_APPLICATION_CREDENTIALS']
        credentials = service_account.Credentials.from_service_account_file(credentials_path)

        # 删除观测日期近7天数据
        query_delete = """
                        delete from airbrush-1324.temp.ads_dz_roi_predict_user_predict_sub_revenue_test
    ...
    gc.collect()
    # evaluate = model_evaluate(d, e_d, stacking, stacking_, xgbr, thred, 0.01)
    # gc.collect()
    ...
    for diff1 in range(0, 3):
        for diff2 in range(0, 13):
            real_predict(stacking, stacking_, xgbr, d, e_d, diff1, diff2, 0.01)
            gc.collect()
