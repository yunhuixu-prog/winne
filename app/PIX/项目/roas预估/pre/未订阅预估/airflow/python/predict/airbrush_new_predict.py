def common_function(ds, **kwargs):
    # py_dws_dz_roi_predict_bp_new_model
    import gc
    import os
    import datetime
    ...
    def real_predict(stacking, xgbr,d, e_d, diff1, diff2, thred_diff):
        ...
        query_real = """
                select *
                from airbrush-1324.temp.dws_dz_roi_predict_final_model_input_delete_v
                where types='new' and date = DATE_SUB({0}, INTERVAL {1} DAY) 
                        and sub_now=0 
                        and days >= 3*{2} and days < 3*({2}+1)
                        and (days<=90 or last_active_days<=60)
        ...
        # airbrush-1324.temp.ads_dz_roi_predict_new_user_predict_sub_revenue
        table_id_user ='temp.ads_dz_roi_predict_new_user_predict_sub_revenue'
        credentials_path = os.environ['GOOGLE_APPLICATION_CREDENTIALS']
        credentials = service_account.Credentials.from_service_account_file(credentials_path)

        # 删除观测日期近7天数据
        query_delete = """
                delete from airbrush-1324.temp.ads_dz_roi_predict_new_user_predict_sub_revenue 
    ...
    e_d='\''+d+'\''
    stacking_ = model_train(d, "'2024-03-20'", 'classfication_second')
    gc.collect()
    stacking, xgbr, thred = model_train(d, "'2024-03-20'", 'others')
    gc.collect()
    ...
    for diff1 in range(0,3):
        for diff2 in range(0, 122):
            real_predict(stacking, xgbr, d, e_d, diff1, diff2, 0.1)
            gc.collect()
