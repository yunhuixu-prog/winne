def common_function(ds, **kwargs):
    ...
    def model_train(d, e_d):
        ...
        query = """
                        ...
                        where types='ua' and Attributed_Touch_Date between DATE_SUB({0}, INTERVAL 458 DAY) and DATE_SUB({0}, INTERVAL 365 DAY)
                            and sub_now=0 and sub_no_trial_365=0 and days between 7 and 365
                            and rand()<0.0004*least(600000/{1},1)
                            """.format(e_d, cnt)
        ...
        return stacking, xgbr, evaluate

    def real_predict(stacking, xgbr, evaluate, d, e_d, diff1, diff2):
        # 真实数据预测(拿历史数据预估，仅测试)
        print('真实样本')
        client = bigquery.Client()
        # 选择投放未满365天的数据
        ...
        y_revenue_365_predict_real.loc[
            y_no_sub_now_predit_real >= thred_real] = y_to_regression_predict_revenue_365_real
    ...
    e_d = '\'' + d + '\''
    stacking, xgbr, evaluate = model_train(d, e_d)
    ...
    for diff1 in range(0, 1):
        for diff2 in range(0, 3):
            real_predict(stacking, xgbr, evaluate, d, e_d, diff1, diff2)
            gc.collect()
