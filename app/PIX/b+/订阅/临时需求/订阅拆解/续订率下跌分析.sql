...
select date
      ,case when date between '2023-12-21' and '2024-01-10' then '2023'
            when date between '2024-12-21' and '2025-01-10' then '2024'
        end date_period
      ,by_period
      ,is_ua
...
(
    ...
    where a.subscription_period='1-year'
    and (end_date between '2023-12-21' and '2024-01-10' or end_date between '2024-12-21' and '2025-01-10')
)
group by 1,2,3,4
order by 1,2,3,4
...
--     a.end_date date
--     ,count(distinct original_order_id) order_num
--     ,count(distinct case when a.id1 is not null then original_order_id end) renewal_order_num
-- from renewal_uv a
-- where a.subscription_period='1-year'
-- and (end_date between '2023-12-21' and '2024-01-10' or end_date between '2024-12-21' and '2025-01-10')
-- group by 1
...
(
    select
        s.end_date,s.start_date,s.Attributed_Touch_Time,
        s.app_id,s.country,
        s.platform,s.subscription_period,s.is_ua,
        s.original_order_id,s.order_id,s.uuid,s.sku,
        if(next_start_date > start_date
...
select
      ...
      ,by_period
      ,is_ua
--       ,case when sub_days<=30 then 'in 30 days' else 'more 30 days' end sub_days
      ,attributed_time
      ,count(distinct original_order_id) order_num
      ,count(distinct case when is_renewal = 1 then original_order_id end) renewal_order_num
from
(
    select
        a.end_date date,a.order_id,a.original_order_id,a.uuid,a.start_date,a.platform,a.subscription_period,a.sku,a.country
        ,d.by_period,a.is_ua
        ,date_diff(start_date,date(Attributed_Touch_Time),day) sub_days
        ,case when date(Attributed_Touch_Time) between '2023-11-29' and '2024-01-10' then '2023-投放'
              when date(Attributed_Touch_Time) between '2022-11-29' and '2023-01-10' then '2022-投放'
        else '其他' end attributed_time
        ,if(a.id1 is not null,1,0) is_renewal
    from renewal_uv a
    left join first_date d
    on a.order_id=d.order_id
...
where by_period=0 and is_ua='non-Organic'
group by 1,2,3,4
order by 1,2,3,4
