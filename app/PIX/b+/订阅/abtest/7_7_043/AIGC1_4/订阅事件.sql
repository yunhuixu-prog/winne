...
(
    ...
    where 
        case    when platform='IOS' then (date>='2024-02-01' and date<='2024-03-07')
                when platform='ANDROID' then (date>='2024-02-22' and date<='2024-03-23')
                end
...
select a.platform
     ...
     ,AVG(coalesce(payment_price_usd,0)) as sub_avg,STDDEV(coalesce(payment_price_usd,0)) sub_std
from ab_user a
left join
(
    select
        code,device_id,sum(payment_price_usd) payment_price_usd
    from ab_sub_event
    where
        standard_order_date is not null and purchase_date is not null
        and event_name in ('subscription_try_suc')
