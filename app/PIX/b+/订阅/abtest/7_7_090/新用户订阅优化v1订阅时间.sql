...
select u.date,u.platform,u.code,round((s.timestamp-u.timestamp)/1000000/60/60) hour
     ,count(distinct u.device_id) sub_uv
     ,count(distinct r.device_id) pay_uv
     ,round(sum(r.payment_price_usd),2) revenue
from
(
select distinct cast(date_p as date) date,platform
...
join
(
    select platform,code,device_id,min(timestamp) timestamp
    from
        ab_sub_event a
    where
...
on u.platform=s.platform and u.code=s.code and u.device_id=s.device_id
left join
(
    select platform,code,device_id,sum(payment_price_usd) payment_price_usd
    from
        ab_sub_event a
    where
...
on u.platform=r.platform and u.code=r.code and u.device_id=r.device_id
group by
    1,2,3,4
