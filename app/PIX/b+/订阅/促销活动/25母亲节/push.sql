...
drop table if exists `dataintegration-265403.temp.mother_push_sub_data_temp`;
create table if not exists `dataintegration-265403.temp.mother_push_sub_data_temp` as
...
(
    ...
    (
        ...
        where event_name = 'notification_open'
            and (`dataintegration-265403.func`.getParams(event_params,'label').string_value like '%PUSH_200%'
                or `dataintegration-265403.func`.getParams(event_params,'label').string_value like '%PUSH_201%'
                or `dataintegration-265403.func`.getParams(event_params,'label').string_value like '%PUSH_202%'
                )
    ) a
    left join
    (
