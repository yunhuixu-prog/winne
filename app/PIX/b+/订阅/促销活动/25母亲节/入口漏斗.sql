...
-- where event_date>='2025-05-09' and module_id='BP_TB_00000008' and event_name in ('home_content_show_f_bd','home_content_clk_bd')
-- group by 1,2,3,4
...
where event_date between '2025-05-09' and '2025-05-13'
  and app_name = 'BeautyPlus'
  and event_name in ('home_content_show_f_bd','home_content_clk_bd')
  and content_type='BP_TB_00000079'
group by 1,2,3,4
...
where event_date_hk between '2025-05-09' and '2025-05-13' and value_name in ('BP_POP_00001671')
group by 1,2,3,4
...
select date
  ,platform
  ,case when source2 in ('BP_POP_00001671') then 'pop'
        when source2='BP_TB_00000079' then 'topbanner'
  end content
...
where date between '2025-05-09' and '2025-05-13' and source2 in ('BP_POP_00001671','BP_TB_00000079') and event_name in ('page_event','subscription_clk_try')
group by 1,2,3,4
...
select date
  ...
  ,count(distinct user_pseudo_id) uv,count(1) pv
from `beautyplus-bc0ed.sub_dataset.ads_spm_trial_subscription_pre_v5_temp`
where date between '2025-05-09' and '2025-05-13' and source2 in ('BP_POP_00001671','BP_TB_00000079') and event_name in ('subscription_try_suc') and standard_order_date is not null
group by 1,2,3,4
...
from `beautyplus-bc0ed.sub_dataset.ads_spm_trial_subscription_pre_v5_temp`
where date between '2025-05-09' and '2025-05-13' and source2 in ('BP_POP_00001671','BP_TB_00000079')
...
--     ,'push' content
--     ,count(distinct user_pseudo_id) uv,count(1) pv
-- from `dataintegration-265403.analytics.dwd_dzp_events_function`('2025-05-09', '2025-05-13', 'BeautyPlus', false)
...
where event_date between '2025-05-09' and '2025-05-13' and app_name='BeautyPlus'
  and push_id in ('PUSH_200','PUSH_201','PUSH_202')
...
select event_date date
  ...
  ,count(distinct user_pseudo_id) uv,count(1) pv
from `dataintegration-265403.temp.mother_push_sub_data_temp`
where event_date between '2025-05-09' and '2025-05-13' and source2 in ('首页-其他入口') and event_name in ('page_event','subscription_clk_try')
group by 1,2,3,4
...
select event_date date
  ...
  ,count(distinct user_pseudo_id) uv,count(1) pv
from `dataintegration-265403.temp.mother_push_sub_data_temp`
where event_date between '2025-05-09' and '2025-05-13' and source2 in ('首页-其他入口') and event_name in ('subscription_try_suc') and standard_order_date is not null
group by 1,2,3,4
...
select event_date date
  ...
  ,count(distinct user_pseudo_id) uv,round(sum(source_amount_proportion*payment_price_usd),2) pv
from `dataintegration-265403.temp.mother_push_sub_data_temp`
where event_date between '2025-05-09' and '2025-05-13' and source2 in ('首页-其他入口') and event_name in ('subscription_try_suc') and standard_order_date is not null and purchase_date is not null
group by 1,2,3,4
...
(
    ...
    (
        ...
        from beautyplus-bc0ed.content_data.dwd_dz_inapp_pop_event
        where event_date_hk between '2025-05-09' and '2025-05-13'
          and value_name in ('BP_POP_00001671')
...
(
    select
        ...
        ,count(distinct case when ((platform='IOS' and `dataintegration-265403.func`.compare_is_greater_or_equal_version(app_version,'7.7.171'))
                                or (platform='ANDROID' and `dataintegration-265403.func`.compare_is_greater_or_equal_version(app_version,'7.7.120'))) then user_pseudo_id end) dau_version
    from
        `dataintegration-265403.stat.stat_active_advice_detail_d`
    where app_name in ('BeautyPlus')
        and event_date_hk between '2025-05-09' and '2025-05-13'
