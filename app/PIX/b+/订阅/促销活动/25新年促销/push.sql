...
drop table if exists `dataintegration-265403.temp.new_year_push_sub_data_temp`;
create table if not exists `dataintegration-265403.temp.new_year_push_sub_data_temp` as
...
(
    ...
    (
        ...
        where event_name = 'notification_open'
            and (`dataintegration-265403.func`.getParams(event_params,'label').string_value like '%PUSH_111%'
                or `dataintegration-265403.func`.getParams(event_params,'label').string_value like '%PUSH_112%'
                or `dataintegration-265403.func`.getParams(event_params,'label').string_value like '%PUSH_114%'
                )
    ) a
    left join
    (
