...
delete from `beautyplus-bc0ed.sub_dataset.ads_spm_trial_subscription_v5_temp`  where date >= '{{ (execution_date + macros.timedelta(hours=8) - macros.timedelta(days=15)).strftime("%Y-%m-%d") }}' ;
--delete from `beautyplus-bc0ed.sub_dataset.ads_spm_trial_subscription_v5_temp`  where date >= '2021-01-01' ;
insert into `beautyplus-bc0ed.sub_dataset.ads_spm_trial_subscription_v5_temp` 
--create table if not exists  `beautyplus-bc0ed.sub_dataset.ads_spm_trial_subscription_v5_temp` as 
with 
...
WHERE a.date >=  '{{ (execution_date + macros.timedelta(hours=8) - macros.timedelta(days=15)).strftime("%Y-%m-%d") }}'
and u.event_date_hk >=  '{{ (execution_date + macros.timedelta(hours=8) - macros.timedelta(days=15)).strftime("%Y-%m-%d") }}'
...
from
 (
  select a.*except(category2,category3_feature_content),
  case
  when a.category3_mid like '1MOC%' then 'Mosaic'
  when a.category3_mid like 'BP_MOC%' then 'Mockup'
  when b.english_name is not null then b.english_name
  when a.category1 ='content' and b.english_name is null then '首页-其他入口'
  else a.category2 end as category2,b.category,
  coalesce(b.category3,a.category3_feature_content) category3_feature_content,
...
case when a.category2 in ('首页-默认入口') then 'content'
  ...
  case
  when a.category3_id like '1MOC%' then 'Mosaic'
  when a.category3_id like 'BP_MOC%' then 'Mockup'
  when b.english_name is not null then b.english_name
  when a.category1 ='content' and b.english_name is null then '首页-其他入口'
  else a.category2 end as category2,b.category,
a.category3_mid,
...
SELECT
    ...
    0 as source_amount_proportion, 
      ...
      where a.event_date_hk >= '{{ (execution_date + macros.timedelta(hours=8) - macros.timedelta(days=15)).strftime("%Y-%m-%d") }}'
      ...
      and b.event_date_hk  >= '{{ (execution_date + macros.timedelta(hours=8) - macros.timedelta(days=15)).strftime("%Y-%m-%d") }}'
      */
