...
delete from `beautyplus-bc0ed.sub_dataset.ads_spm_trial_subscription_pre_v5_abtest` where date >= '{{ (execution_date + macros.timedelta(hours=8) - macros.timedelta(days=15)).strftime("%Y-%m-%d") }}';    
...
--create table if not exists  `beautyplus-bc0ed.sub_dataset.ads_spm_trial_subscription_pre_v5_abtest` as 
...
--     split(a.pre_page1,'@')[0] = b.page_id
  ...
  LEFT JOIN
      `finance-268602.dim.dim_da_subscription_sku_info`  s --2022/11/11换成这个表，因为原表的halloween sku信息更改后无法同步
  ON
...
--         WHEN a.source1 IN ('content') AND (c like '%BP_KKAA%' or c like '%ABVC_BP%')  THEN 'HomePage Topbar'
    ...
    END) --category2 string
      ...
      --AND c LIKE 'BP%' THEN REGEXP_EXTRACT(c, r"(?:1|2|BP\_|BP\-|BP)(?:[A-Z]{3})[\_\-]?(.*)")
        WHEN a.source1 IN ('content') and (c like '%BP_POP%' or c like '%BP_TB%') THEN c--拆分首页内容   or c like '%BP_KKAA%' or c like '%ABVC_BP%'
    ...
    END --category3_mid string
      ,
      CASE
        WHEN a.source1 IN ('content') and (c like '%BP_POP%' or c like '%BP_TB%' or a.source2 like '%HPP%'or a.source2 like '%LBD%') THEN '' --  or c like '%BP_KKAA%' or c like '%ABVC_BP%'
        ...
        WHEN a.source1 IN ('content') and `dataintegration-265403.func`.compare_is_greater_or_equal_version(app_version,'7.5.095') THEN SPLIT(c, ',')[SAFE_OFFSET(2)]  --新首页的内容id
