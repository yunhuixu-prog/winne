...
,fe_save as ( -- 限制进入实验的人,且实验触发日期在进入实验之后
    select b.code,b.device_id,count(1) value
    from eves a
    join enter_test b on a.device_id= b.device_id
    where a.event_timestamp>=b.timestamp-15000000
    group by 1,2
...
select a.code,'sub' types,count(distinct a.device_id) computer_uv,round(AVG(coalesce(b.value,0)),6) mean,round(STDDEV(coalesce(b.value,0)),4) std
...
select code,'save' types,count(distinct device_id) computer_uv,round(AVG(coalesce(value,0)),6) mean,round(STDDEV(coalesce(value,0)),4) std
from fe_save
group by 1,2
...
-- where event_date_hk='2025-03-23'  -- 是否要限制观测日期
group by 1
ORDER BY 1
