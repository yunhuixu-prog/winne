...
select source,sum(active_users),sum(beauty_uv),sum(beauty_feature_or_shop_uv),sum(beauty_feature_or_shop_or_home_uv),
  sum(search_uv),sum(search_clk) search_clk,sum(search_result) search_result,sum(material_imp),sum(material_clk),sum(material_save),
  sum(trending_word_clk),sum(search_content_clk),sum(trending_search),sum(subscription_try_suc)
from `beautyplus-bc0ed.event_dataset.dwd_search_behavior_overall`
where event_date='2023-12-01'
group by 1


select distinct source,material_type,source_material_type
from `beautyplus-bc0ed.temp.dwd_search_behavior_t`
where event_date>='2023-10-01'
and event_name='homepageappr_bd'


-- 2023.10.01-2023-10.31
drop table if exists `beautyplus-bc0ed.temp.dwd_search_behavior_t`;
create table if not exists `beautyplus-bc0ed.temp.dwd_search_behavior_t` as

-- delete from `beautyplus-bc0ed.event_dataset.dwd_search_behavior`
-- where event_date>='2023-10-01'
--     and event_date<='2023-10-31';
-- insert into `beautyplus-bc0ed.event_dataset.dwd_search_behavior`
...
(
    select
        ...
        ,version
    from
        `dataintegration-265403.analytics.dwd_dzp_events_function`('2023-10-01',
        '2023-10-31','beautyplus',true)
...
(
    ...
    where
        event_date_hk between date'2023-10-01'
        and '2023-10-31'
...
select
    ...
    ,event_name
    ,t.platform platform
    ,case   when event_name in ('beautifysave_bd','subscription_try_suc','beauty_appr_bd','beau_sticker_save_bd','homepageappr_bd') then 'total'
...
where
    case    when event_name in ('beauty_template_material_appr_bd','beauty_template_material_clk_bd','beautifysave_bd') then tem_tag='BP_cat_TEM_SCH'
            ...
            when event_name in ('beauty_sticker_imp_bd','beau_clk_sticker_use_bd','beau_sticker_save_bd') then sticker_tag='BP_cat_DST_SCH'
...
(
    ...
    from
        beautyplus-bc0ed.temp.dwd_search_behavior_t
    where event_date >= '2023-10-01'
        and event_date <= '2023-10-31'
        and event_name not in ('homepageappr_bd')
    group by
        1,2,3,4,5,6
    ...
    select
        country
        ,'home_page_search' source
        ,platform
        ,'all' source_material_type
        ,event_date
        ,event_name
        ,sum(pv) pv
        ,count(distinct user_pseudo_id) uv
...
(
    ...
    from
        beautyplus-bc0ed.temp.dwd_search_behavior_t s
        left join   (select
                        country
                        ,platform
                        ,word_content
    ...
    where s.event_date >= '2023-10-01'
        and s.event_date <= '2023-10-31'
        and event_name in ('material_search_content_bd')
    group by
        1,2,3,4,5
)
...
(
    ...
    from
        `dataintegration-265403.stat.stat_active_advice_detail_d`
    where
        event_date_hk between '2023-10-01'
        and '2023-10-31'
        and app_name='BeautyPlus'
    group by
        1,2,3
