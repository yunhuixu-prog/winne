...
--     p.country
    ...
    ,sum(case when event_name in ('beauty_template_material_appr_bd','beauty_sticker_imp_bd','search_miniapp_appr_bd') then pv end) material_imp
    ,sum(case when event_name in ('beauty_template_material_clk_bd','beau_clk_sticker_use_bd','search_miniapp_clk_bd') then pv end) material_clk
