...
-- drop table if exists `dataintegration-265403.temp.dwd_ds_xyz_project_behavior`;
-- create table if not exists `dataintegration-265403.temp.dwd_ds_xyz_project_behavior` as

delete from  `dataintegration-265403.temp.dwd_ds_xyz_project_behavior`  where event_date>='{{ (execution_date + macros.timedelta(hours=8) - macros.timedelta(days=3)).strftime("%Y-%m-%d") }}' and event_date<='{{ (execution_date + macros.timedelta(hours=8) - macros.timedelta(days=0)).strftime("%Y-%m-%d") }}';
insert into `dataintegration-265403.temp.dwd_ds_xyz_project_behavior`
...
(
   select
        ...
        ,coalesce(`dataintegration-265403.func`.getParams(event_params,'entry').string_value,
                  `dataintegration-265403.func`.getParams(event_params,'from_page').string_value) entry
...
--         ,`dataintegration-265403.func`.getParams(event_params,'photo_num').string_value photo_num  --如果有多个风格，记得除一下
        ...
        ,case when app_name = 'AirBrush' and ARRAY_LENGTH(split(`dataintegration-265403.func`.getParams(event_params,'onelink_source').string_value,'='))>=2
              then split(`dataintegration-265403.func`.getParams(event_params,'onelink_source').string_value,'=')[1]
        ...
        ,user_pseudo_id
    from
        `dataintegration-265403.analytics.dwd_dzp_events_function`('{{ (execution_date + macros.timedelta(hours=8) - macros.timedelta(days=3)).strftime("%Y-%m-%d") }}', '{{ (execution_date + macros.timedelta(hours=8) - macros.timedelta(days=0)).strftime("%Y-%m-%d") }}', 'beautyplus,airbrush,beautypluscam', true)
...
--         ,user_pseudo_id
--     from
--         `dataintegration-265403.analytics.dwd_dzp_events_function`('{{ (execution_date + macros.timedelta(hours=8) - macros.timedelta(days=3)).strftime("%Y-%m-%d") }}', '{{ (execution_date + macros.timedelta(hours=8) - macros.timedelta(days=0)).strftime("%Y-%m-%d") }}', 'airbrush', true)
--     where event_name in ('edit_save' -- ab编辑器
--                         )
)
,
event_pre as
(
    select *
    from event_pre_raw
    where case when event_name in ('home_content_show_f_bd','home_content_clk_bd') then (module_type='miniapp' or (module_type='Banner' and content_type in ('miniapp','H5')) or (module_type in ('推荐功能','Topbanner','XYZ') and content_type in (select distinct Material_id from `dataintegration-265403.temp.dwd_da_miniapp_material_id_mapping` where App!='AirBrush')))
            ...
            -- save上报异常
            when event_name in ('h5_page_button_clk_bd','h5_page_button_clk') and button_type = 'save'
                    and project = 'ai_filter' and event_date>='2024-12-31' and theme_type='photo' then save_type is not null
            when event_name in ('h5_page_button_clk_bd','h5_page_button_clk') then button_type in ('non_zero_generate_upload','zero_generate_upload','non_zero_generate','zero_generate','generate','list','save','save_all','save_video','save_gif','share','image','frame','retry','upload_new','to_video') and project in (select distinct H5_name from `dataintegration-265403.temp.dwd_da_miniapp_miniapp_name_mapping`)
            when event_name in ('h5_home_content_show_f','h5_home_content_clk','h5_home_content_show_f_bd','h5_home_content_clk_bd','h5_credit_consume_bd','h5_credit_consume','h5_page_visit_bd','h5_page_visit') then project in (select distinct H5_name from `dataintegration-265403.temp.dwd_da_miniapp_miniapp_name_mapping`)
    ...
    select
        ...
        ,case   when event_name in ('home_content_show_f_bd','home_content_clk_bd','home_page_pop_appr_bd','home_page_pop_clk_bd','search_miniapp_appr_bd','search_miniapp_clk_bd','popup_show','popup_click') then m.miniapp
                when event_name in ('h5_page_event_bd','h5_page_event','h5_page_button_clk_bd','h5_page_button_clk','h5_home_content_show_f','h5_home_content_clk','h5_home_content_show_f_bd','h5_home_content_clk_bd','h5_credit_consume_bd','h5_credit_consume','h5_page_visit_bd','h5_page_visit') then s.miniapp
        ...
        ,case when event_name in ('h5_page_event_bd','h5_page_event','h5_page_button_clk_bd','h5_page_button_clk'
                                ...
                                ,'h5_credit_consume_bd','h5_credit_consume','h5_page_visit_bd','h5_page_visit') then 'H5'
              ...
              when event_date<='2024-11-24' and e.miniapp_content_id in ('BP_POP_00001610','BP_POP_00001611') then 'H5'
              when event_name in ('home_content_show_f_bd','home_content_clk_bd','home_page_pop_appr_bd','home_page_pop_clk_bd'
                                ...
                                ,'homepage_func_show','homepage_func_click','link_app_start_bd','link_app_start') then coalesce(m.hp_source,'Unknown')
        end source
        ,page_id
        ,button_type
        ,module_type
    ...
    (
        select
            ...
            ,event_name
            ,project
            ,case when event_name in ('h5_page_event_bd','h5_page_event','h5_page_button_clk_bd','h5_page_button_clk','h5_home_content_show_f','h5_home_content_clk','h5_home_content_show_f_bd','h5_home_content_clk_bd','h5_credit_consume_bd','h5_credit_consume','h5_page_visit_bd','h5_page_visit') then entry else null end entry
            ,func
            ,pop_id
    ...
    left join (select Material_id,App,max(Project) miniapp,max(case when Source='编辑器' then 'Style' when Source='配方' then 'Template' else Source end) hp_source from `dataintegration-265403.temp.dwd_da_miniapp_material_id_mapping` group by 1,2) m on e.miniapp_content_id=m.Material_id
