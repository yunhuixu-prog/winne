-- !!!替换日期的时候，别把下面的时间条件给替换掉了
-- drop table if exists `dataintegration-265403.temp.dws_ds_xyz_project_behavior_overall`;
-- create table if not exists `dataintegration-265403.temp.dws_ds_xyz_project_behavior_overall` as

delete from  `dataintegration-265403.temp.dws_ds_xyz_project_behavior_overall`  where date>='{{ (execution_date + macros.timedelta(hours=8) - macros.timedelta(days=14)).strftime("%Y-%m-%d") }}' and date<='{{ (execution_date + macros.timedelta(hours=8) - macros.timedelta(days=0)).strftime("%Y-%m-%d") }}';
insert into `dataintegration-265403.temp.dws_ds_xyz_project_behavior_overall`
...
--         ,sum(case when (event_name in ('h5_page_event_bd','h5_page_event') and page_id in ('homepage','home_page_view')) or event_name in ('beauty_appr_edit_clk_bd','first_func_enter') then pv end) visit_pv
        ,sum(case when (event_name in ('h5_page_event_bd','h5_page_event') and page_id in ('homepage','home_page_view') and event_date<='2024-11-18')
                                      ...
                                      or (event_name in ('h5_page_visit_bd','h5_page_visit') and event_date>'2024-11-18' and project not in ('AI_Pet_Portray'))
                                      or (event_name in ('h5_page_visit_bd','h5_page_visit') and event_date>'2024-12-13' and project in ('AI_Pet_Portray'))
                                      or event_name in ('beauty_appr_edit_clk_bd','first_func_enter') then pv end) visit_pv
    ...
    from
        `dataintegration-265403.temp.dwd_ds_xyz_project_behavior` e,unnest(split(coalesce(theme,''),',')) k
...
(
    select app_name,event_date date,platform,country,is_new,is_UA,project_name,entry,source
            ...
            ,visit_pv,enter_generate_page_pv,generate_pv,save_pv,share_pv,onelink_pv,generate_photo_num,save_photo_num
    ...
    select app_name,date,platform,country,is_new,is_UA,project_name,entry,source
            ...
            ,0 visit_pv,0 enter_generate_page_pv,0 generate_pv,0 save_pv,0 share_pv,0 onelink_pv,0 generate_photo_num,0 save_photo_num
            ,sub_uv,sub_pay_uv,sub_revenue
    from subscription

    union all

    select app_name,date,platform,country,is_new,is_UA,project_name,'All' entry,'All' source
            ,0 exposure_uv,0 click_uv
            ,0 exposure_miniapp_uv,0 click_miniapp_uv
            ,0 exposure_banner_uv,0 click_banner_uv
            ,0 exposure_function_uv,0 click_function_uv
