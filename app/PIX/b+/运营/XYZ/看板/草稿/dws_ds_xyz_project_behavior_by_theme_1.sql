...
drop table if exists `dataintegration-265403.temp.t_dws_ds_xyz_project_behavior_by_theme`;
create table if not exists `dataintegration-265403.temp.t_dws_ds_xyz_project_behavior_by_theme` as
...
(
    select
        ...
        ,'All' entry
        ,source
        ,case when source in ('H5') and project in ('ai_filter','ai_portrait') then theme_type
              when source in ('H5') and project in ('puriplus') then machine
              when source in ('Template','Style') then 'photo'
        else 'All'
        end theme_type
        ,theme
        ,case when source in ('Template','Style') then material_id else theme end theme_id
...
(
    select app_name,event_date date,platform,country,is_new,is_UA,project_name,entry,source,theme_type,theme,theme_id
            ...
            ,enter_generate_page_pv,generate_pv,save_pv,share_pv
    ...
    select app_name,date,platform,country,is_new,is_UA,project_name,entry,source,theme_type,theme,theme_id
            ...
            ,0 enter_generate_page_pv,0 generate_pv,0 save_pv,0 share_pv
