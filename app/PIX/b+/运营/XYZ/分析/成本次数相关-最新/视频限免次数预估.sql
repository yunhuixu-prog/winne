...
(
    ...
    where
        event_date_hk between '2025-01-01' and '2025-06-24'
        and app_name in ('BeautyPlus','AirBrush')
    group by 1,2,3,4,5
)
,creative_user as
(
    select e.app_name,e.event_date,e.project_name,e.user_pseudo_id,sum(e.pv) generate_pv
...
select app_name,project_name,generate_pv_1day,generate_pv_31day,count(distinct user_pseudo_id) uv
from
(
    select f.app_name,f.project_name,f.user_pseudo_id
        ,sum(case when c.event_date=f.first_generate_date then generate_pv end) generate_pv_1day
        ,sum(generate_pv) generate_pv_31day
    from
    (
        select app_name,project_name,user_pseudo_id,min(event_date) first_generate_date
        from creative_user
        group by 1,2,3
        having min(event_date) <= date_sub('2025-06-24',interval 30 day)
    ) f
    left join creative_user c
    on f.app_name=c.app_name and f.project_name=c.project_name and f.user_pseudo_id=c.user_pseudo_id
