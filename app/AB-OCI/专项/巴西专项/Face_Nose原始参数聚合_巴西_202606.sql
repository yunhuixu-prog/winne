-- AirBrush / 2026-06 / 巴西 / Face-Nose 原始参数字符串聚合
SELECT
    '巴西' AS market_name,
    'Nose' AS feature_name,
    TRIM(source_event.params['prf_nose_mod']) AS raw_mod,
...
FROM (
    ...
    WHERE e.date_p BETWEEN 20260601 AND 20260630
      ...
      AND TRIM(COALESCE(e.params['prf_nose_mod'], '')) <> ''
) source_event
INNER JOIN (
    SELECT
        active_user.date_p,
...
GROUP BY
    TRIM(source_event.params['prf_nose_mod'])
