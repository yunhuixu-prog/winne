...
SOURCE = Path(__file__).with_name("巴西专项_功能渗透率_新老用户_202606.json")
...
def render(segment_cn):
    rows = [r for r in json.loads(SOURCE.read_text(encoding="utf-8"))["rows"] if r["用户类型"] == segment_cn]
    top_rows, rank_maps = {}, {}
    ...
    for i in range(20):
        ...
        for j, market in enumerate(DISPLAY_MARKETS):
            ...
            if market != "整体":
                ...
                overall_value = next(
                    float(r["进入渗透率"]) for r in rows
                    if r["国家维度"] == "整体" and r["功能"] == feature
                )
                value_gap = value - overall_value
                ...
                if value_gap > 0.02 and peer_gap >= peer_threshold:
                    accent = value_color = COLORS["ahead"]
                elif value_gap < -0.02 and peer_gap <= -peer_threshold:
