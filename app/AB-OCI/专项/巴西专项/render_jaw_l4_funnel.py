...
SOURCE = (
    ...
    "巴西专项_Face_Jaw四级功能漏斗_202606.csv"
)
FACE_SOURCE = (
    ROOT
    / "outputs/019f839e-6d3f-7a81-b0ac-450473a2e302/"
...
def footnote_text(denominators):
    return (
        f"Face-Jaw 进入：巴西 {denominators['巴西']:,.0f} 人/日，"
        f"整体 {denominators['整体']:,.0f} 人/日；巴西明细有效27日，整体30日。"
        "Jaw默认打开，且会记忆上次停留子项，因此进入占比不能直接视为主动偏好。"
    )


def main():
    renderer.OUTPUT = OUTPUT
    renderer.TITLE = "Face－Jaw 四级功能｜进入与效果确认漏斗"
    renderer.SUBTITLE = (
        "按巴西四级功能进入人数降序；每列展示 gap（巴西－整体）及巴西/整体具体值。"
    )
    renderer.FEATURE_HEADER = "Jaw 四级功能"
    renderer.FEATURE_SUBHEADER = "巴西四级功能进入人数（日均 UV）"
    renderer.METRICS = ["进入占 Jaw", "进入打勾率", "进入保存率"]
    renderer.HEADER_SUBTITLES = {
        "进入占 Jaw": "四级功能进入 / Face-Jaw 进入",
        "进入打勾率": "四级功能打勾 / 四级功能进入",
        "进入保存率": "四级功能保存 / 四级功能进入",
    }
