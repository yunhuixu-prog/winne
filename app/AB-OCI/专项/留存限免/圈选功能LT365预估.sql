select a.function_name
    ...
    ,sum(retention_1_uv_blocked)/sum(blocked_uv) retention_1_uv_blocked_rate
    ,sum(retention_2_uv_blocked)/sum(blocked_uv) retention_2_uv_blocked_rate
    ,sum(retention_4_uv_blocked)/sum(blocked_uv) retention_4_uv_blocked_rate
    ,sum(retention_5_uv_blocked)/sum(blocked_uv) retention_5_uv_blocked_rate
    ,sum(retention_6_uv_blocked)/sum(blocked_uv) retention_6_uv_blocked_rate
    ,sum(retention_7_uv_blocked)/sum(blocked_uv) retention_7_uv_blocked_rate
    ,sum(retention_1_uv_no_blocked)/sum(no_blocked_uv) retention_1_uv_no_blocked_rate
    ,sum(retention_3_uv_no_blocked)/sum(no_blocked_uv) retention_3_uv_no_blocked_rate
    ,sum(retention_4_uv_no_blocked)/sum(no_blocked_uv) retention_4_uv_no_blocked_rate
    ,sum(retention_5_uv_no_blocked)/sum(no_blocked_uv) retention_5_uv_no_blocked_rate
    ,sum(retention_6_uv_no_blocked)/sum(no_blocked_uv) retention_6_uv_no_blocked_rate
    ,sum(retention_7_uv_no_blocked)/sum(no_blocked_uv) retention_7_uv_no_blocked_rate
    ,sum(retention_1_uv_save)/sum(save_uv) retention_1_uv_save_rate
    ,sum(retention_2_uv_save)/sum(save_uv) retention_2_uv_save_rate
    ,sum(retention_3_uv_save)/sum(save_uv) retention_3_uv_save_rate
    ,sum(retention_4_uv_save)/sum(save_uv) retention_4_uv_save_rate
    ,sum(retention_5_uv_save)/sum(save_uv) retention_5_uv_save_rate
    ,sum(retention_6_uv_save)/sum(save_uv) retention_6_uv_save_rate
    ,sum(retention_7_uv_save)/sum(save_uv) retention_7_uv_save_rate
from (
select
    a.date_p
    ...
    ,count(distinct case when coalesce(f.enter_pv,0)>0 and coalesce(sf.is_sub_enter,0)>=1 
        then a2.gid end) retention_2_uv_blocked
    ,count(distinct case when coalesce(f.enter_pv,0)>0 and coalesce(sf.is_sub_enter,0)=0 
        then a2.gid end) retention_2_uv_no_blocked
    ,count(distinct case when coalesce(f.save_pv,0)>0
        then a2.gid end) retention_2_uv_save

    ,count(distinct case when coalesce(f.enter_pv,0)>0 and coalesce(sf.is_sub_enter,0)>=1 
        then a3.gid end) retention_3_uv_blocked
    ,count(distinct case when coalesce(f.enter_pv,0)>0 and coalesce(sf.is_sub_enter,0)=0 
        then a3.gid end) retention_3_uv_no_blocked
    ,count(distinct case when coalesce(f.save_pv,0)>0
        then a3.gid end) retention_3_uv_save
    
    ,count(distinct case when coalesce(f.enter_pv,0)>0 and coalesce(sf.is_sub_enter,0)>=1 
        then a4.gid end) retention_4_uv_blocked
    ,count(distinct case when coalesce(f.enter_pv,0)>0 and coalesce(sf.is_sub_enter,0)=0 
        then a4.gid end) retention_4_uv_no_blocked
    ,count(distinct case when coalesce(f.save_pv,0)>0
        then a4.gid end) retention_4_uv_save
    
    ,count(distinct case when coalesce(f.enter_pv,0)>0 and coalesce(sf.is_sub_enter,0)>=1 
        then a5.gid end) retention_5_uv_blocked
    ,count(distinct case when coalesce(f.enter_pv,0)>0 and coalesce(sf.is_sub_enter,0)=0 
        then a5.gid end) retention_5_uv_no_blocked
    ,count(distinct case when coalesce(f.save_pv,0)>0
        then a5.gid end) retention_5_uv_save
    
    ,count(distinct case when coalesce(f.enter_pv,0)>0 and coalesce(sf.is_sub_enter,0)>=1 
        then a6.gid end) retention_6_uv_blocked
    ,count(distinct case when coalesce(f.enter_pv,0)>0 and coalesce(sf.is_sub_enter,0)=0 
        then a6.gid end) retention_6_uv_no_blocked
    ,count(distinct case when coalesce(f.save_pv,0)>0
        then a6.gid end) retention_6_uv_save
    
    ,count(distinct case when coalesce(f.enter_pv,0)>0 and coalesce(sf.is_sub_enter,0)>=1 
        then a7.gid end) retention_7_uv_blocked
    ,count(distinct case when coalesce(f.enter_pv,0)>0 and coalesce(sf.is_sub_enter,0)=0 
        then a7.gid end) retention_7_uv_no_blocked
    ,count(distinct case when coalesce(f.save_pv,0)>0
        then a7.gid end) retention_7_uv_save
from (
    SELECT
        a.date_p,
        a.os_p,
...
-- on a.gid=a1.gid and a.date_p=a.date_p=CAST(FROM_UNIXTIME(UNIX_TIMESTAMP(CAST(a1.date_p AS STRING), 'yyyyMMdd') - 86400, 'yyyyMMdd') AS BIGINT)
on a.gid=a2.gid and a.date_p=CAST(date_format(date_sub(from_unixtime(unix_timestamp(cast(a2.date_p as string), 'yyyyMMdd')), 2), 'yyyyMMdd') AS BIGINT)
left join (
    SELECT date_p, final_id gid
    FROM stat_sdk.sdk_odz_active
    WHERE date_p BETWEEN 20260101 AND ${now_date}
...
-- on a.gid=a1.gid and a.date_p=a.date_p=CAST(FROM_UNIXTIME(UNIX_TIMESTAMP(CAST(a1.date_p AS STRING), 'yyyyMMdd') - 86400, 'yyyyMMdd') AS BIGINT)
on a.gid=a3.gid and a.date_p=CAST(date_format(date_sub(from_unixtime(unix_timestamp(cast(a3.date_p as string), 'yyyyMMdd')), 3), 'yyyyMMdd') AS BIGINT)
left join (
    SELECT date_p, final_id gid
    FROM stat_sdk.sdk_odz_active
    WHERE date_p BETWEEN 20260101 AND ${now_date}
...
-- on a.gid=a1.gid and a.date_p=a.date_p=CAST(FROM_UNIXTIME(UNIX_TIMESTAMP(CAST(a1.date_p AS STRING), 'yyyyMMdd') - 86400, 'yyyyMMdd') AS BIGINT)
on a.gid=a4.gid and a.date_p=CAST(date_format(date_sub(from_unixtime(unix_timestamp(cast(a4.date_p as string), 'yyyyMMdd')), 4), 'yyyyMMdd') AS BIGINT)
left join (
    SELECT date_p, final_id gid
    FROM stat_sdk.sdk_odz_active
    WHERE date_p BETWEEN 20260101 AND ${now_date}
...
-- on a.gid=a1.gid and a.date_p=a.date_p=CAST(FROM_UNIXTIME(UNIX_TIMESTAMP(CAST(a1.date_p AS STRING), 'yyyyMMdd') - 86400, 'yyyyMMdd') AS BIGINT)
on a.gid=a5.gid and a.date_p=CAST(date_format(date_sub(from_unixtime(unix_timestamp(cast(a5.date_p as string), 'yyyyMMdd')), 5), 'yyyyMMdd') AS BIGINT)
left join (
    SELECT date_p, final_id gid
    FROM stat_sdk.sdk_odz_active
    WHERE date_p BETWEEN 20260101 AND ${now_date}
...
-- on a.gid=a1.gid and a.date_p=a.date_p=CAST(FROM_UNIXTIME(UNIX_TIMESTAMP(CAST(a1.date_p AS STRING), 'yyyyMMdd') - 86400, 'yyyyMMdd') AS BIGINT)
on a.gid=a6.gid and a.date_p=CAST(date_format(date_sub(from_unixtime(unix_timestamp(cast(a6.date_p as string), 'yyyyMMdd')), 6), 'yyyyMMdd') AS BIGINT)
left join (
    SELECT date_p, final_id gid
    FROM stat_sdk.sdk_odz_active
    WHERE date_p BETWEEN 20260101 AND ${now_date}
...
-- on a.gid=a1.gid and a.date_p=a.date_p=CAST(FROM_UNIXTIME(UNIX_TIMESTAMP(CAST(a1.date_p AS STRING), 'yyyyMMdd') - 86400, 'yyyyMMdd') AS BIGINT)
on a.gid=a7.gid and a.date_p=CAST(date_format(date_sub(from_unixtime(unix_timestamp(cast(a7.date_p as string), 'yyyyMMdd')), 7), 'yyyyMMdd') AS BIGINT)
