select
    a.os_type os_type,
    case when b.ab_code in ('28525','28522') then '对照组'
         when b.ab_code in ('28526','28523') then '实验组A'
         when b.ab_code in ('28527','28524') then '实验组B'
        end code
    ,case when b.country in ('澳大利亚','巴西','英国','美国') then b.country else '其他' end country
--     ,b.enter_abtest_date
    -- 付费
    ,a.duration duration
...
from (
    ...
    where
        date_p between 20251204 and 20251223
    ...
    (
        ...
        from (
            ...
            (
                ...
                WHERE date_p BETWEEN 20251204 and 20251223
        ...
        join (
            ...
            WHERE date_p between 20251204 and 20251223
                ...
                AND params['current_abcode'] in ('28525','28526','28527','28522','28523','28524')
...
group by a.os_type,case when b.ab_code in ('28525','28522') then '对照组'
         when b.ab_code in ('28526','28523') then '实验组A'
         when b.ab_code in ('28527','28524') then '实验组B'
        end
        ,case when b.country in ('澳大利亚','巴西','英国','美国') then b.country else '其他' end
        ,a.duration
...
select
    ...
    ,case when a.ab_code in ('28525','28522') then '对照组'
        when a.ab_code in ('28526','28523') then '实验组A'
        when a.ab_code in ('28527','28524') then '实验组B'
        end code
    ,case when a.country in ('澳大利亚','巴西','英国','美国') then a.country else '其他' end country
--     ,b.enter_abtest_date
    -- 付费
    ,'无' duration
    -- ,'无' source_0
...
group by case when a.os_type = 'ios' then 'iOS'
    ...
    ,case when a.country in ('澳大利亚','巴西','英国','美国') then a.country else '其他' end
