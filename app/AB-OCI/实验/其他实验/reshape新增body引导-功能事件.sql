...
select
    -- a.date_p,
    b.os_type os_type
    ,case when b.ab_code in ('28950') then '对照组'
        when b.ab_code in ('28951') then '实验组A'
        when b.ab_code in ('28952') then '实验组B'
        end code
    ,b.is_new is_new
    ...
    (
        ...
        join (
            ...
            WHERE date_p between 20260519 and 20260623
                ...
                AND params['current_abcode'] in ('28950','28951','28952')
...
group by b.os_type,case when b.ab_code in ('28950') then '对照组'
        when b.ab_code in ('28951') then '实验组A'
        when b.ab_code in ('28952') then '实验组B'
        end
        ,b.is_new
