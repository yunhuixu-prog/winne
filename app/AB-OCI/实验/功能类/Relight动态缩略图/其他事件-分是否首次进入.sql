insert overwrite table stat_ab.filing_ona_relight_abtest_temp
select
    -- a.date_p,
    -- a.os_type os_type
    ...
    ,a.event_id,a.gid,a.event_timestamp
from (
...
;
select a.code,b.event_id,count(distinct a.gid) uv
from (
    select code,gid,event_timestamp
    from (
        select code,gid,event_timestamp
            ,row_number() over(partition by gid order by event_timestamp) ranks
        from stat_ab.filing_ona_relight_abtest_temp
        where event_id='second_func_enter'
    ) a
    where ranks=2
) a 
join (
    select code,gid,event_id,event_timestamp
    from stat_ab.filing_ona_relight_abtest_temp
) b 
on a.gid= b.gid and b.event_timestamp >= a.event_timestamp
group by a.code,b.event_id
