...
insert overwrite table stat_vip.vip_adz_middle_income_dayoci PARTITION(date_p=${start_time})
select
     /*+MAPJOIN(t4)*/
...
from
    (select  nvl(os_type,'整体')  as os_type
            ...
            ,sum(case when pay_date=${start_time} then ord_amt else 0 end) as gmv_day    -- 每天收入
            ,sum(case when refund_date=${start_time} and pay_status=6 then refund_amt else 0 end) as refund_gmv_day     -- 每天退款
            ,sum(case when substr(pay_date,1,4)=${start_year} then ord_amt else 0 end) as gmv_year    -- 202x年累计收入
            ,sum(case when substr(refund_date,1,4)=${start_year} and pay_status=6 and refund_date <=${start_time} then refund_amt else 0 end) as refund_gmv_year    -- 202x年累计退款
            ...
            ,count(distinct case when cur_withhold_stage=1 and pay_date=${start_time} then gid else null end) as new_member           -- 新增会员（含免费试用)
            ,count(distinct case when cur_pay_withhold_stage=1 and pay_date=${start_time} then gid else null end) as new_pay_member   -- 新增付费会员
            ,count(distinct case when cur_withhold_stage=1 and pay_date=${start_time} then notify_pay_id else null end) as new_notifyid         -- 新增订单（含免费试用)
            ,count(distinct case when cur_pay_withhold_stage=1 and pay_date=${start_time} then notify_pay_id else null end) as new_pay_notifyid  -- 新增付费订单
            ,count(distinct case when cur_pay_withhold_stage>=2 and pay_date=${start_time} then gid else null end) as  renew_member   -- 续费会员数
            ,count(distinct case when cur_pay_withhold_stage>=1 and pay_date=${start_time} then gid else null end) as  pay_member   -- 付费会员数
            ,count(distinct valid_gid ) as valid_menber  -- 每日有效付费会员数（付费状态）
            ,count(distinct valid_dau_gid) as valid_menber_dau  -- 每日有效付费活跃会员数（付费状态）
            ,sum(case when pay_date=${start_time} then ord_amt_usd else 0 end) as gmv_day_usd
            ,sum(case when refund_date=${start_time} and pay_status=6 then refund_amt_usd else 0 end) as refund_gmv_day_usd     -- 每天退款
            ,sum(case when substr(pay_date,1,4)=${start_year} then ord_amt_usd else 0 end) as gmv_year_usd    -- 202x年累计收入
            ,sum(case when substr(refund_date,1,4)=${start_year} and pay_status=6 and refund_date <=${start_time} then refund_amt_usd  else 0 end) as refund_gmv_year_usd    -- 202x年累计退款
            ,sum(ord_amt_usd) as gmv_all_usd   -- 项目累计收入
    from
          (select
           /*+MAPJOIN(t22)*/
               notify_pay_id
               ...
               ,case when type='订阅' and invalid_date>=${start_time} and cur_pay_withhold_stage>=1 and pay_date<=${start_time} then gid
                     when type='单购'  and cur_pay_withhold_stage>=1 and pay_date =${start_time} then gid
                   else null end as valid_gid
               ,case when type='订阅' and invalid_date>=${start_time} and cur_pay_withhold_stage>=1 and pay_date<=${start_time} and t2.server_id is not null then gid
                     when type='单购'  and cur_pay_withhold_stage>=1 and pay_date =${start_time}  and t2.server_id is not null then gid
          ...
          left join
              ...
              WHERE app_key_p in ('C4E808751210527E','B8FD6DF9B9E02ACF','F9B069901A7B2E8D', 'C6FF0769324CD2F1')
    ...
    group by os_type,product_sub_line,country_code,period_type,pay_channel,type,geographic_subdivision_v2,ocean_name  with cube
    )t3
left join
    (select nvl(app_name,'整体') as   app_name
        ...
        ,nvl(geographic_subdivision_v2,'整体') as geographic_subdivision_v2
     	,nvl(ocean_name ,'整体') as ocean_name
        ,count(distinct server_id) as dau
      from
        (select app_name
            ,os_p
