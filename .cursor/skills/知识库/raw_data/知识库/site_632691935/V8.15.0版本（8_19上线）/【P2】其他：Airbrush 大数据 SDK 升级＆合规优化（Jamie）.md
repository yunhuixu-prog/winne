# 【P2】其他：Airbrush 大数据 SDK 升级＆合规优化（Jamie）

**页面ID**: 710772695

**路径**: V8.15.0版本（8_19上线）/【P2】其他：Airbrush 大数据 SDK 升级＆合规优化（Jamie）
...
- 二次确认后，弹窗提示已排定的执行时间：**冷静期由 D+1 调整为 D+3**，文案沿用「Queued for deletion. Expected to complete by YYYY/MM/DD HH:00.」，若为非0分0秒，自动进位到整点
- 到达预定时间节点后，才正式触发删除与 gid 刷新；冷静期内再次点击 Delete my data，沿用现有「已在队列中，Expected to complete by YYYY/MM/DD HH:00」toast 提示
- 冷静期目的：gid 刷新会重置绑定的免费次数，拉长冷静期以提高灰黑产套利
- gid 刷新后的预期
...
| 事件名 | 触发时机 | 参数 ||
| delete_data_window_exp | 资料删除二次确认弹窗曝光 | 
 ||
| delete_data_window_click | 资料删除二次确认弹窗点击 | btn_name: delete / cancel ||
| delete_data_queue_window_exp | 排程提示弹窗曝光（展示 D+3 预定执行时间） | 
 ||
| gid_refresh_success | 到达预定时间节点、gid 刷新完成（服务端 / SDK 上报） | 与服务端 gid 记录表对账，用于滥用检测 ||
| withdraw_consent_btn_click | More Option 页点击撤回授权同意按钮 | 
 ||
| withdraw_consent_window_exp | 撤回同意二次确认弹窗曝光 | 
 ||
| withdraw_consent_window_click | 撤回同意二次确认弹窗点击 | btn_name: confirm / cancel ||
| privacy_window_exp | 授权同意弹窗曝光 | source: withdraw / new_user ||
| privacy_window_click | 授权同意弹窗点击 | btn_name: agree / disagree ||