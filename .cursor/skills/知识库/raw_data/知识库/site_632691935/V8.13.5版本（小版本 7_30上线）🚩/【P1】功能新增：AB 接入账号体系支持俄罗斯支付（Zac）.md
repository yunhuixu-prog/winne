...
**路径**: V8.13.5版本（小版本 7_30上线）🚩/【P1】功能新增：AB 接入账号体系支持俄罗斯支付（Zac）
...
| 2026.6.14 | Zac | 内审后更新 | 
 ||
| 2026.6.28 | Zac | 根据账号中台功能更新 | 
 ||
| 2026.7.20 | Zac | 更新 iOS 登录页审核逻辑 | 
 ||
...
- **当用户自主登录时（点击 Settings/Log in）**
- **弹出半屏登录组件**

- **当强制用户登录时（俄罗斯用户点击 pay wall 中的 subscribe）**
- **弹出登录页**

**7/20 更新：新增 iOS 登录账号审核逻辑**
**审核期间：
**

- **当用户自主登录时（点击 Settings/Log in）及 当强制用户登录时（俄罗斯用户点击 pay wall 中的 subscribe）时**
- 默认是邮箱登录（全屏），Apple 展示在第三方登录的第一个item
...
| 
 | **Premium 卡片优化 - 更新
**优化现 Premium 卡片信息展示，隐藏 My Coins 入口并移入 login 组件中。缩小 Premium 卡片。 ||
| 
 | **Account 页面 - 需跟随中台逻辑**
入口：
Settings
...
Something went wrong. Please try again.
 ||
| 
 | **Account / Profile / Edit Name - 需跟随中台逻辑**
...
- 保存失败时，停留当前页，并提示：Something went wrong. Please try again.
 ...
 ||
| 
 | **Account / Profile / Edit Gender - 需跟随中台逻辑**
...
- 服务端返回字段异常时，不更新本地展示，并提示稍后重试。
 ...
 ||
| 
 | **Account / Profile / Edit Birthday - 需跟随中台逻辑**
...
- 如果后续需要展示年龄，年龄由 Birthday 计算得出，不由用户手动输入。
 ...
 ||
| 
 | **Account / Connected Accounts 页面 - 需跟随中台逻辑**
**中台支持链接，断链等操作**
...
若 Log out 请求失败，保留当前登录态，提示用户稍后重试。
 ||
| 
 | **Delete Account - 需跟随中台逻辑**

- **需要与中台确认「分账号删除」是否能跟随版本上线**
- **删除页的最后需修改文案**
...
- 用户触发登录流程（需区分地区）
- 来源（设置页，paywall）
- 用户点击具体登录方式（VK/邮箱/Apple/Google/FB）
- 用户取消登录
- 用户登录成功
- 用户登录失败
...
- 用户查看当前登录账号信息
- 用户点击连接其他平台账号
- 用户选择具体连接方式，例如 VK / Apple / Google / Facebook / 邮箱
- 第三方账号连接成功
- 第三方账号连接失败
- 用户取消第三方账号连接
- 用户尝试解绑已连接的第三方账号
...
### 十一.UI
Figma链接：[https://www.figma.com/design/ZofUiWLUGo5Cui901ajV5C/Settings---Adjust?node-id=28201-9665&t=DG1yP53hiuds5fSH-1](https://www.figma.com/design/ZofUiWLUGo5Cui901ajV5C/Settings---Adjust?node-id=28201-9665&t=DG1yP53hiuds5fSH-1)