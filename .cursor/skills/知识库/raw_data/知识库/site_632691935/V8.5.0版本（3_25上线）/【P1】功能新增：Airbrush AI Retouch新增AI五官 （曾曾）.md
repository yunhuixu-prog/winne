...
**路径**: V8.5.0版本（3_25上线）/【P1】功能新增：Airbrush AI Retouch新增AI五官 （曾曾）
...
- 切换tab 使用再点击undo时， 需要依次回到上个/上上个结果-->回到原图
...
- 画面和UI一致，滑杆显示历史该历史节点的参数强度
b：当前 UI 不是 undo 对应的效果
- 默认选中上一个效果，但UI 不跳转，当前效果项取消选中态；若用户点开上个对应效果，可以基于当前结果图继续编辑；若为跨 Tab 场景执行 undo，当前 Tab 内的效果会被取消，并自动选中「none」，当用户切回上一 Tab 时，将恢复显示该 Tab 的历史结果图，同时保持上一效果的选中状态，可直接继续调节参数。
- 多人脸场景下：
...
- 切换人脸并应用效果后，再次切回原人脸仍可继续调节滑杆参数。
同一人脸若先后使用不同 Tab 下的子效果（如先使用鼻子、再使用眉毛），切回鼻子时需重新请求，且请求基于之前已叠加的效果（鼻子 + 眉毛） 计算。
