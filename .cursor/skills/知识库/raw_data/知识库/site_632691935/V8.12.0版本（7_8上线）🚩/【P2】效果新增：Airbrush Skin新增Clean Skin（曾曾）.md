...
**路径**: V8.12.0版本（7_8上线）🚩/【P2】效果新增：Airbrush Skin新增Clean Skin（曾曾）
...
| 原型图 | 需求描述 ||
| 
 | **入口：Retouch- Skin-Clean Skin（第四位）**
**效果参数：（待设计师@莫策 明确）**

- Clean Skin-100%，效果参数包如下：
- 秀秀无暇：100%
- AB:Dark Circles Auto 45%
- AB:Wrinkles Auto 30%
...
- Clean Skin上方新增New角标，用户点击后消失

**交互流程**

- 进入Skin，Clean Skin默认关闭，不选中任何子项
- 点击Clean Skin，首次需要下载模型，follow已有交互（若在用户进入前已经下载完成则不需要再次下载）
- 进入功能后默认选中auto并效果生效展示滑杆，默认100%，长按收起滑杆
- 同skin其他功能，支持笔刷、笔刷大小、橡皮擦功能
...
- 俄罗斯用户：
- 限免次数>0，正常扣除次数，限免次数=0，打钩触发俄罗斯广告弹窗
...
## 八、埋点需求
**Clean Skin**的曝光/点击/打勾/保存/订阅的UV/PV埋点