...
**路径**: V8.6.0版本（4_8上线）/【P1】功能新增：Airbrush Eraser 新增杂物消除（Jamie）
...
| 算法接口 | 杂物分割
/v1/img_sundries_seg_async ＋ /v1/mtimage_remove_flux_async
AI消除 flux版 文档： 
...
- 弹出时机：**「首次」**点击杂物图标时，需要展示。功能引导弹窗展示时，暂缓投递。关掉弹窗后开始投递杂物分割算法。
...
- 弹出时机：**「首次」**点击镜面图标时，需要弹出。功能引导弹窗展示时，暂缓投递。关掉弹窗后开始投递镜面去污算法。
...
- 不会展示限免横幅，展示效果后，展示Pro横幅，打勾&无限免次数都直接触发展示【订阅页】，也不会有需求里提到的【兜底图】提示
