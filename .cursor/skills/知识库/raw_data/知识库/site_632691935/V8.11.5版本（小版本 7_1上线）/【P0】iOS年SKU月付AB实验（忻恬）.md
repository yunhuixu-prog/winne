...
| 2026.6.22 | 忻恬 | 年商品展示名统一为 Annual，补充商品卡片下方营销文案取值逻辑。 | 评审调整 ||
...
- 在 Airbrush paywall 中新增 Annual（billed monthly）SKU，首次上线通过 AB 实验验证用户接受度。
- 目标提升订阅按钮点击率和付费转化率，重点观察 Annual 一次性付费犹豫用户是否会转向年承诺月付。
- 实验不以替代 prepaid Annual 为目标，prepaid Annual 仍保留最低总价心智；Annual（billed monthly）用于测试更低首期支付门槛。

### 3、需求说明：
本期作为 AB 实验接入，实验组展示 3 个 SKU：Annual（prepaid）、Annual（billed monthly）、Monthly。
...
默认选中分期 Annual
 ||
| 
- 沿用当前线上订阅页商品结构：Annual / Monthly 的价格、试用开关、支付说明均保持线上逻辑。
- 用于对比实验组新增年承诺月付后的转化、付费结构和投诉风险。

 | 
- 两个年 SKU 均展示为 **Annual**。
- prepaid Annual 主价格按线上逻辑展示 **$4.58/mo**，并展示 **Save 65%**。
- Annual（billed monthly）仅作为备选，展示 **Billed monthly**，不展示 Save 角标。

 | 
- 两个年 SKU 均展示为 **Annual**。
- prepaid Annual 不展示角标，展示全年价格 **$54.99/yr**。
- Annual（billed monthly）展示 **Save 46%**，并默认选中。
...
商品卡片下方说明文案取值逻辑：端上优先读取订阅中台商品【营销文案】字段；若未配置【营销文案】，走原本展示划线价/原价对比的逻辑。
商品价格取值逻辑：端上优先读取订阅中台商品【商品价格展示文案】字段；若未配置【商品价格展示文案】，走原本商品价格展示逻辑（根据当前入口下配置的最小时长单位SKU进行换算）。
**二、Annual（billed monthly）SKU展示前提：**

- 分期不是新商品类型，而是在既有 yearly auto-renewable subscription 上增加 monthly billing plan；订阅中台按「付费方案」区分预付和分期。
- 订阅产品会在 airbrush.paywall.newuser、airbrush.paywall.olduser 的常态入口&对应的用户分层入口配置该实验SKU，且仅在【巴西、英国、澳大利亚】地区的入口配置。
- 本期仅巴西、英国、澳大利亚3个国家上线分期月付SKU。将仅针对以上3个地区下发分期商品。airbrush.paywall.newuser、airbrush.paywall.olduser 将分化出针对这3个地区的入口。
...
- 分期月付 SKU 首次上线不配置 7-day free trial，仅配置无试用的 Annual billing plan（分期月付）。
- 用户选中 Annual（分期月付）时，不展示 7-Day Free Trial 开关；CTA 固定展示 **Continue**。
- Annual / Monthly 沿用线上试用逻辑，不因本实验调整。

#### 四、**Annual（billed monthly）SKU商品说明文案**

| 状态 | CTA文案 | 商品说明文案 ||
| 无试用 | Continue | %1$s/mo for 12 payments. Renews automatically, cancel any time. ||
| 有试用
（本期虽不上线试用，但预埋文案）
 | Try for Free | %1$s/mo for 12 payments. Renews automatically after %2$s-day trial, cancel any time.
 ||
