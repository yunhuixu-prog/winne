...
说明：
  ...
  本脚本参考 `参考文档/分析下钻报告实现过程/计算脚本/2数据处理/s5异常检测.py`
  的异常判定规则，按当前源数据可用周数输出异常检测表。
"""
...
METRIC_SPECS: list[MetricSpec] = [
    ...
    MetricSpec(
        ...
        numerator_column="次日留存人数",
        denominator_column="活跃用户数",
        filters={"渠道自然": "整体", "国家": "整体", "平台": "整体", "新老": "整体"},
        fmt="pct",
    ),
    MetricSpec(
        name="整体新增次留",
        file_name="new_retention.csv",
        numerator_column="新增次日留存人数",
        denominator_column="DNU",
        filters={"渠道自然": "整体", "国家": "整体", "平台": "整体"},
        fmt="pct",
    ),
    MetricSpec(
...
def fetch_weekly(spec: MetricSpec, history_weeks: int) -> pd.DataFrame:
    """返回按周升序排列的 date/value 明细。"""
    ...
    if spec.value_column:
        ...
        weekly = grouped.rename(columns={"__日期__": "date"})[["date", "value"]]

    weekly = weekly.dropna(subset=["value"]).sort_values("date")
    # 历史周期：截断到 ZHOUBAO_TARGET_WEEK（含）为止
    import sys
    ...
    if target is not None:
        ...
        weekly = weekly[weekly["date"] <= target]
    return weekly.tail(history_weeks).reset_index(drop=True)


def make_week_label(week_start: pd.Timestamp) -> str:
    week_start = pd.Timestamp(week_start)
    week_end = week_start + pd.Timedelta(days=6)
    return f"wk{week_start.isocalendar().week}<br>{week_start:%m%d}-{week_end:%m%d}"
...
def pre_3w_stable(history: list[float]) -> bool:
    if len(history) < 5:
        return False
...
def detect_anomaly(display_vals: list[float], raw_hist: list[float], name: str) -> tuple[str, str]:
    ...
    cur = display_vals[-1]
    prev = display_vals[-2]
    wow_pct = (cur - prev) / prev * 100 if prev != 0 else 0.0
    abs_wow = abs(wow_pct)
    history_label = f"近{len(raw_hist)}周最低值"
    ...
    if consec >= 2 and is_low:
        desc.append(f"{name}连续{consec + 1}周下跌且达{history_label}")
    elif consec >= 2:
        desc.append(f"{name}连续{consec + 1}周下跌")
    elif is_low:
    ...
    if not desc and pre_3w_stable(display_vals) and abs_wow > 5:
        desc.append(f"{name}连续三周稳定后骤然下跌")
    if not desc:
        if abs_wow > 5:
...
def status_icon(status: str, wow: float) -> str:
    if status == "增长":
        return "🟢"
    if status == "异常":
        return "🔴"
    if status == "稳定":
        return "🟡" if wow <= 0 else "⚫"
    return ""


def build_report(display_weeks: int, history_weeks: int) -> str:
    rows: list[dict[str, Any]] = []
    week_dates: list[pd.Timestamp] = []

    for spec in METRIC_SPECS:
        weekly = fetch_weekly(spec, history_weeks)
        if len(weekly) < 2:
            continue

        display = weekly.tail(display_weeks)
        dates = [pd.Timestamp(d) for d in display["date"].tolist()]
        values = [float(v) for v in display["value"].tolist()]
        history_values = [float(v) for v in weekly["value"].tolist()]
        prev = values[-2]
        wow = (values[-1] - prev) / prev * 100 if prev != 0 else 0.0
        status, desc = detect_anomaly(values, history_values, spec.name)

        if len(dates) > len(week_dates):
            week_dates = dates
    ...
    labels = [make_week_label(d) for d in week_dates]
    title_weeks = len(labels)
    table = "| 指标 | " + " | ".join(labels) + " | 周环比变化 | 指标状态 | 异常说明 |\n"
    table += "|" + "---|" * (title_weeks + 4) + "\n"
    ...
    for row in rows:
        ...
        line += f" {fmt_pct(row['wow'])} |"
        line += f" {status_icon(row['status'], row['wow'])} |"
    ...
    note = (
        ...
        f"最低值判断基于当前源数据可用的最近最多{history_weeks}周历史。*"
    )
    return f"# 异常指标检测（近{title_weeks}周）\n\n{table}\n{note}"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="生成 AI 周报异常指标检测 Markdown")
    parser.add_argument("--display-weeks", type=int, default=DEFAULT_DISPLAY_WEEKS)
...
def main() -> None:
    args = parse_args()
    if args.display_weeks < 2:
        raise ValueError("--display-weeks 至少为 2")
    if args.history_weeks < args.display_weeks:
        raise ValueError("--history-weeks 不能小于 --display-weeks")

    print("生成异常指标检测MD文档...")
    md = build_report(args.display_weeks, args.history_weeks)
